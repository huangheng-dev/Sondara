...
export function IcpPage() {
  ...
  const selectedMarketName = profile?.selectedMarket || ''
  ...
  const markets: Market[] = analysis?.recommendedMarkets?.map((item, index) => ({
    ...
    profile: (analysis.criteria ?? []).slice(0, 3),
  })) ?? []
  ...
  const criteriaRows = (analysis?.criteria ?? []).slice(0, 5)
  const profileItems = [
  ...
  return <PageContainer>
    ...
    <Row className="icp-result-grid" gutter={[16, 16]} aria-label="客户定位结果">
      ...
      <Col xs={24} xl={16}>
        <Panel className="icp-portrait-panel" title={`${selectedMarket?.name ?? '待选择市场'} · 理想客户画像`} subtitle="系统自动归纳，无需手动设置评分参数" action={<Badge tone={selectedMarket?.status === '优先开发' ? 'green' : selectedMarket?.status === '继续验证' ? 'orange' : 'neutral'}>{selectedMarket?.status}</Badge>}>
          <Flex vertical gap={16}>
            <Card className="icp-conclusion-card" size="small" title={<Space size={8}><Sparkles size={17}/>AI 定位结论</Space>}><Typography.Text strong>{analysisRunning ? '正在重新分析业务资料…' : analysis?.summary || '等待生成定位结论'}</Typography.Text><Typography.Paragraph type="secondary">{analysisRunning ? '正在结合产品、案例和市场资料生成最新结果。' : (selectedReason || '完成分析后会显示该市场的推荐依据。')}</Typography.Paragraph></Card>
            <Row className="icp-stat-grid" gutter={[10,10]}>
              ...
              <Col xs={12} md={6}><Card size="small"><Statistic title="重点信号" value={analysis?.signals?.length??0} suffix="项"/></Card></Col>
            ...
            <Flex className="icp-portrait-actions" justify="flex-end" wrap gap={8}><Button onClick={() => setMarketDetail(selectedMarket ?? null)}>查看完整画像</Button><Button variant="primary" disabled={analysisRunning || !selectedMarket} onClick={() => selectedMarket && startRadar(selectedMarket)}>按此定位找客户<ArrowRight size={14} /></Button></Flex>
          </Flex>
        </Panel>
      </Col>
    </Row>
    ...
    <Modal open={Boolean(marketDetail)} title={`${marketDetail?.name ?? ''} · 理想客户画像`} description="以下结论由你的业务资料自动生成，AI 获客会据此判断匹配度。" onClose={() => setMarketDetail(null)} footer={<><Button onClick={() => setMarketDetail(null)}>关闭</Button><Button variant="primary" onClick={() => marketDetail && startRadar(marketDetail)}>按此画像找客户</Button></>}>
      <Space orientation="vertical" size="large" style={{ width: '100%' }}>
        <Row gutter={[16,16]}>
          ...
          <Col span={8}><Statistic title="重点信号" value={analysis?.signals?.length??0} suffix="项"/></Col>
          <Col span={8}><Statistic title="筛选条件" value={criteriaRows.length} suffix="项"/></Col>
        </Row>
