...
export function LeadSourcesPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return <PageContainer>
    ...
    <Panel title="线索入口" subtitle="官网表单、Webhook 和广告线索统一在这里配置；每个真实连接拥有独立 Token、处理策略和运行状态。" action={embedded ? <Space><ListRefreshButton label="检查新线索" onClick={refresh} loading={connections.isFetching || events.isFetching}/><Button variant="primary" disabled={!canManageSettings} onClick={() => openCreate()}><Plus size={16}/>添加其他来源</Button></Space> : undefined}>
      {connections.isLoading ? <EmptyState spinning title="正在读取线索来源" icon={RefreshCw}/> : connections.isError ? <EmptyState title="线索来源读取失败" description={connections.error instanceof Error ? connections.error.message : '请稍后重试。'} icon={Link2} action={<Button size="sm" onClick={() => connections.refetch()}>重新加载</Button>}/> : <DataTable ariaLabel="线索入口配置表格" columns={[{ key: 'name', title: '来源', width: 260 }, { key: 'provider', title: '接入方式与准备项', width: 320 }, { key: 'automation', title: '自动化闭环', width: 320 }, { key: 'status', title: '连接状态', width: 150 }, { key: 'synced', title: '最近事件', width: 180 }, { key: 'actions', title: '操作', width: 96 }]} rows={[
    ...
    <Panel title="线索处理记录" subtitle="每个事件都会显示是否已经完成客户、联系人和任务闭环。">
      {events.isLoading ? <EmptyState spinning title="正在读取线索事件" icon={RefreshCw}/> : events.isError ? <EmptyState title="线索事件读取失败" description={events.error instanceof Error ? events.error.message : '请稍后重试。'} icon={Webhook}/> : events.data?.items.length ? <DataTable ariaLabel="线索处理记录表格" columns={[{ key: 'event', title: '事件', width: 320 }, { key: 'status', title: '处理状态', width: 150 }, { key: 'result', title: '闭环结果', width: 340 }, { key: 'time', title: '接收时间', width: 180 }, { key: 'action', title: '查看', width: 72 }]} rows={events.data.items.map(item => ({ key: item.id, cells: [
