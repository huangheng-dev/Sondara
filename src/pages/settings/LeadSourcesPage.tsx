...
export function LeadSourcesPage() {
  ...
  return <div>
    <PageHeader title="官方线索渠道" description="接收 LinkedIn Lead Gen 和 Meta Lead Ads 的官方表单线索；仅支持获得平台授权的账户。" actions={<><Button onClick={refresh} disabled={connections.isFetching||events.isFetching}><RefreshCw size={16} className="is-spinning"/>刷新</Button><Button variant="primary" onClick={() => setCreateOpen(true)}><Plus size={16}/>添加渠道</Button></>}/>
    <Panel><div><header><span><strong>官方线索连接</strong><small>配置后复制 Webhook 地址到平台；平台回调会先进入待处理事件，不会直接自动外发。</small></span></header>{connections.isLoading ? <EmptyState spinning title="正在读取渠道连接" icon={RefreshCw}/> : connections.isError ? <EmptyState title="渠道连接读取失败" description={connections.error instanceof Error ? connections.error.message : '请稍后重试。'} icon={Link2} action={<Button size="sm" onClick={()=>connections.refetch()}>重新加载</Button>}/> : connections.data?.items.length ? <DataTable columns={[{ key: 'name', title: '渠道' }, { key: 'target', title: '账户与表单' }, { key: 'token', title: '凭据' }, { key: 'status', title: '状态' }, { key: 'action', title: 'Webhook' }]} rows={connections.data.items.map(item => ({ key: item.id, cells: [<div><strong>{item.name}</strong><small>{providerLabel[item.provider]}</small></div>, <div><strong>{item.accountRef ?? '待填写广告账户/Page ID'}</strong><small>{item.formRef ?? '未限定表单'}</small></div>, <Badge tone={item.hasAccessToken ? 'green' : 'orange'}>{item.hasAccessToken ? `Token · •••• ${item.accessTokenEnding}` : '待配置 Token'}</Badge>, <Badge tone={item.status === 'webhook_received' ? 'green' : item.status === 'ready_for_authorization' ? 'blue' : 'orange'}>{item.status === 'webhook_received' ? '已收到事件' : item.status === 'ready_for_authorization' ? '待平台授权' : '待配置'}</Badge>, <Button size="sm" onClick={() => void rotateWebhook(item)}><Webhook/>重置地址</Button>] }))}/> : <EmptyState title="尚未接入官方线索渠道" description="先添加 LinkedIn Lead Gen 或 Meta Lead Ads，保存平台凭据后再到对应平台订阅 Webhook。" icon={Link2}/>}</div></Panel>
    <Panel><div><header><span><strong>最近收到的线索事件</strong><small>平台事件已去重保存；填写官方访问凭据后将用于拉取完整表单字段并入库。</small></span></header>{events.isLoading ? <EmptyState spinning title="正在读取线索事件…" icon={RefreshCw}/> : events.isError ? <EmptyState title="线索事件读取失败" description={events.error instanceof Error ? events.error.message : '请稍后重试。'} icon={Webhook} action={<Button size="sm" onClick={()=>events.refetch()}>重新加载</Button>}/> : events.data?.items.length ? <DataTable columns={[{ key: 'event', title: '事件' }, { key: 'status', title: '状态' }, { key: 'time', title: '接收时间' }]} rows={events.data.items.slice(0, 20).map(item => ({ key: item.id, cells: [<div><strong>{item.providerEventId}</strong><small>{item.connectionId}</small></div>, <Badge tone={item.processingStatus === 'received' ? 'blue' : 'orange'}>{item.processingStatus === 'received' ? '待拉取线索详情' : item.processingStatus}</Badge>, <span>{new Date(item.receivedAt).toLocaleString('zh-CN')}</span>] }))}/> : <EmptyState title="尚未收到平台事件" description="完成平台 Webhook 订阅后，新的表单提交会显示在这里。" icon={Webhook}/>}</div></Panel>
