...
import { Alert, Form, Input, InputNumber, Popconfirm, Space, Switch, Tabs, Typography } from 'antd'
...
const connectorSummary: Record<string, string> = {
  'company-contact-database': '补全目标企业和公开商务联系人',
  'email-verification': '核验商务邮箱的可达性与风险',
  'phone-verification': '识别国际号码类型、地区与有效性',
  'trade-supply-chain-data': '查询进出口、提单与供应链关系',
  'crm-sync': '同步客户、联系人、任务与商机',
  'website-visitor-identification': '识别网站访问企业并形成意向信号',
  'vertical-industry-database': '接入行业企业、项目与专业名录',
}
...
export function ConnectorCatalogPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({ name: selected.configuration?.name ?? selected.name, providerName: selected.examples[0], ...defaults, ...selected.configuration?.settings })
  }, [form, selected])
  ...
  const save = async () => {
    ...
    try {
      ...
      await integrationApi.saveExternalConnector(selected.key, { name: String(values.name || selected.name), enabled: selected.configuration?.enabled ?? false, settings, credentials })
  ...
  const toggleConnector = async (item: ExternalConnectorCatalogItem, enabled: boolean) => {
    const config = item.configuration
    if (!config) return
    setTogglingKeys(current => new Set(current).add(item.key))
    try {
      await integrationApi.saveExternalConnector(item.key, { name: config.name, enabled, settings: {}, credentials: {} })
      await catalog.refetch()
      showToast(enabled ? `${item.name}已启用` : `${item.name}已停用`)
    } catch (cause) {
      showToast(cause instanceof Error ? cause.message : '启用状态更新失败')
    } finally {
      setTogglingKeys(current => { const next = new Set(current); next.delete(item.key); return next })
    }
  }
  ...
  const extensionRows = filtered.map(item => {
    ...
    return { key: item.key, cells: [
      <Space orientation="vertical" size={1} key="name"><Typography.Text strong>{item.name}</Typography.Text><Typography.Text type="secondary">{connectorSummary[item.key] ?? item.description}</Typography.Text></Space>,
      ...
      <Switch key="enabled" aria-label={`${item.name}启用状态`} title={config ? (config.enabled ? '停用连接器' : '启用连接器') : '请先完成配置'} checked={config?.enabled ?? false} disabled={!config} loading={togglingKeys.has(item.key)} onChange={enabled => toggleConnector(item, enabled)}/>,
      <Space key="actions">
  ...
  const extensionContent = catalog.isLoading ? <PageState status="loading" title="正在读取可选集成"/> : catalog.isError ? <PageState status="error" title="可选集成读取失败" onRetry={() => catalog.refetch()}/> : <Panel title="可选外部服务" subtitle="按业务需要填写自己的服务商账号和密钥；不配置不会影响内置能力。"><TableToolbar filters={<><SearchInput ariaLabel="搜索可选集成" value={query} onChange={event => setQuery(event.target.value)} placeholder="搜索能力或服务商"/><CustomSelect ariaLabel="筛选可选集成分类" value={category} onChange={setCategory} options={['全部分类', ...Object.values(categoryLabel)]}/></>}/><DataTable ariaLabel="可选外部服务表格" minWidth={1380} columns={[{ key: 'name', title: '集成能力', width: 280 }, { key: 'category', title: '分类', width: 150 }, { key: 'description', title: '用途', width: 360 }, { key: 'status', title: '配置状态', width: 190 }, { key: 'enabled', title: '启用', width: 90 }, { key: 'actions', title: '操作', width: 310 }]} rows={extensionRows}/></Panel>
  ...
  return <PageContainer>
    ...
    <Modal open={Boolean(selected)} width={680} title={`配置${selected?.name ?? ''}`} description="密钥和 Token 通过 AES-256-GCM 加密保存；留空的密钥字段会保留已有值。" onClose={() => setSelected(null)} footer={<><Button onClick={() => setSelected(null)}>取消</Button><Button variant="primary" loading={saving} onClick={save}>加密保存</Button></>}>
      {selected ? <Form form={form} layout="vertical" requiredMark={renderRequiredOrOptionalMarkAfter}>
        <Form.Item name="name" label="连接名称" rules={[{ required: true, message: '请输入连接名称' }]}><Input/></Form.Item>
        {selected.fields.map(field => <Form.Item key={field.key} name={field.key} label={field.secret && selected.configuration?.credentialEndings[field.key] ? `${field.label}（当前 •••• ${selected.configuration.credentialEndings[field.key]}）` : field.key === 'providerName' ? '服务商' : field.label} rules={[{ required: Boolean(field.required && !(field.secret && selected.configuration?.hasCredentials)), message: `请填写${field.key === 'providerName' ? '服务商' : field.label}` }]}>
          {field.key === 'providerName'
            ? <CustomSelect ariaLabel="服务商" options={selected.examples}/>
            : field.type === 'password'
