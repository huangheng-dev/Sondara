...
export function CandidateList({ candidates, saved, selected, sort, onSortChange, onSelectionChange, onOpen, onSave }: CandidateListProps) {
  ...
  return <DataTable
    ...
    columns={[
      ...
      { key: 'company', title: <Button onClick={() => onSortChange(sort === '企业名称 A–Z' ? '企业名称 Z–A' : '企业名称 A–Z')}>企业档案{sortIcon(sort === '企业名称 A–Z' || sort === '企业名称 Z–A', sort === '企业名称 Z–A')}</Button>, width: 300, fixed: 'left' },
      ...
      { key: 'research', title: <Button onClick={() => onSortChange(sort === '预计价值最高' ? '预计价值最低' : '预计价值最高')}>信号与价值{sortIcon(sort === '预计价值最高' || sort === '预计价值最低', sort === '预计价值最高')}</Button>, width: 260 },
    ...
    rows={candidates.map(candidate => ({
      ...
      cells: [
        ...
        <Space orientation="vertical" size={2}><Badge tone={/新建|扩张|招投标/.test(candidate.signal) ? 'green' : 'blue'}>{candidate.signal}</Badge><Typography.Text type="secondary" ellipsis={{tooltip:candidate.source}}>{candidate.source}</Typography.Text><Flex gap={8}><Typography.Text strong>{candidate.value}</Typography.Text><Typography.Text type="secondary">· {candidate.evidence.length} 条证据</Typography.Text></Flex><Button type="link" onClick={()=>onSortChange(sort === '最近发现' ? '最早发现' : '最近发现')}>发现于 {candidate.updatedAt}{sortIcon(sort === '最近发现' || sort === '最早发现', sort === '最近发现')}</Button></Space>,
        <Button type="link" aria-label={`查看 AI 决策：${candidate.company}`} onClick={() => onOpen(candidate)}><FileSearch2/><Space orientation="vertical" size={0}><Typography.Text strong>{candidate.score >= 92 ? '优先跟进' : '查看研究结论'}</Typography.Text><Typography.Text type="secondary" ellipsis>{candidate.reason}</Typography.Text></Space></Button>,
