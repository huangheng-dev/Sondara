  ...
  return <DataTable
    ...
    rows={candidates.map(candidate => ({
      ...
      cells: [
        ...
        <Space className="table-signal-cell" orientation="vertical" size={3}><Badge tone={/新建|扩张|招投标/.test(candidate.signal) ? 'green' : 'blue'} truncate tooltip={candidate.signal}>{candidate.signal}</Badge><Typography.Text type="secondary" ellipsis={{tooltip:candidate.source}}>{candidate.source}</Typography.Text></Space>,
