...
export function RadarPage() {
  ...
  const candidateQuery=useQuery({queryKey:['radar-candidates',workspaceId],queryFn:()=>radarApi.listCandidates({pageSize:100,sort:'updated_desc'}),enabled:Boolean(workspaceId),retry:1,refetchInterval:3000})
  ...
  return <div className="page-content radar-page">
      ...
      <Panel className="customer-workspace radar-customer-workspace">
        <div className="customer-toolbar radar-toolbar module-toolbar standard-list-toolbar">
          <div className="customer-filter-controls radar-filter-controls">
            ...
            <Button className="customer-refresh" onClick={async()=>{await Promise.all([candidateQuery.refetch(),radarTaskQuery.refetch()]);showToast('候选客户列表已刷新')}}><RefreshCw/>刷新</Button>
            <Button className="module-clear" disabled={!query&&filter==='全部'&&mode==='智能多渠道'&&sort==='匹配分最高'} onClick={()=>{setQuery('');setFilter('全部');setMode('智能多渠道');setSort('匹配分最高')}}>清除筛选</Button>
          </div>
          <div className={`customer-selection-tools${checkedCandidates.size>0?' has-selection':' is-empty'}`} aria-hidden={checkedCandidates.size===0}><span><CheckCircle2/><small>已选择</small><strong>{checkedCandidates.size}</strong><small>家</small></span><div><Button onClick={saveChecked}><Bookmark/>保存客户</Button><Button onClick={()=>setBatchTaskOpen(true)}><CheckCircle2/>创建任务</Button><Button aria-label="取消选择" title="取消选择" onClick={()=>setCheckedCandidates(new Set())}><X/></Button></div></div>
        </div>
        <CandidateList candidates={candidatePaging.pageItems} saved={saved} selected={checkedCandidates} sort={sort} onSortChange={setSort} onSelectionChange={setCheckedCandidates} onOpen={setSelected} onSave={save} />
    ...
    <CompanyDecisionDrawer candidate={selected} open={Boolean(selected)} onClose={() => setSelected(null)} onSave={save} onEnrich={enrichCandidate} onCreateTask={createCandidateTask} />
