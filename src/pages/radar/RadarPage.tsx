...
import { authApi, automationApi, customerApi, icpApi, radarApi, taskApi, type AcquisitionPlanStatus, type CustomerApiRecord, type RadarTaskStatus } from '@/lib/api'
...
export function RadarPage() {
  ...
  const currentPlan=plans.find(plan=>plan.status==='active')??plans.find(plan=>plan.status==='blocked')??plans[0]??null
  const performanceQuery=useQuery({queryKey:['acquisition-plan-performance',workspaceId,currentPlan?.id],queryFn:()=>radarApi.planPerformance(currentPlan!.id,30),enabled:Boolean(workspaceId&&currentPlan?.id),retry:1,refetchInterval:30_000})
  const performance=performanceQuery.data
  const learningQuery=useQuery({queryKey:['acquisition-plan-learning',workspaceId,currentPlan?.id],queryFn:()=>radarApi.planLearning(currentPlan!.id,90),enabled:Boolean(workspaceId&&currentPlan?.id),retry:1,refetchInterval:30_000})
  ...
  const activeLearningVersion=learningVersionsQuery.data?.items.find(item=>item.status==='active')
  ...
  const candidateQuery=useQuery({queryKey:['radar-candidates',workspaceId,archiveView,filteredTaskId??'all'],queryFn:()=>radarApi.listCandidates({pageSize:100,sort:'updated_desc',taskId:filteredTaskId,status:archiveView?'archived':undefined}),enabled:Boolean(workspaceId),retry:1,refetchInterval:3000})
  const taskCandidateQuery=useQuery({queryKey:['radar-task-candidates',workspaceId,activeTask?.id],queryFn:()=>radarApi.listCandidates({pageSize:100,sort:'updated_desc',taskId:activeTask?.id}),enabled:Boolean(workspaceId&&activeTask?.id&&runHistoryOpen),retry:1,refetchInterval:runHistoryOpen?3000:false})
  ...
  useEffect(()=>{setCheckedCandidates(new Set())},[candidateTaskId,archiveView])
  ...
  const updateCurrentPlan=async(values:Record<string,string>)=>{if(!currentPlan)return;const scheduleType=values.schedule==='每天自动'?'daily':values.schedule==='每周一自动'?'weekly':values.schedule==='仅运行一次'?'manual':'weekdays';const fullAutopilot=values.automation==='全自动开发（安全发送）';await radarApi.updatePlan(currentPlan.id,{name:values.name,targetRegion:values.region,candidateLimit:Number(values.limit),dailyCandidateLimit:Number(values.limit),scheduleType,runTimeLocal:values.runTime,weekdays:scheduleType==='weekly'?[1]:[1,2,3,4,5],automationMode:values.automation==='自动研究'?'research_only':'safe_autopilot',autoOutreachEnabled:fullAutopilot});await planQuery.refetch()}
  const changeGlobalAutomation=async(action:'pause_all'|'resume_all')=>{try{const result=await radarApi.setAutomationControl(action);await Promise.all([productionQuery.refetch(),planQuery.refetch(),performanceQuery.refetch()]);showToast(action==='pause_all'?`自动触达已暂停，取消 ${result.cancelledMessages??0} 条待发送消息`:`已恢复 ${result.affectedPlans??0} 个全自动计划`)}catch(cause){showToast(cause instanceof Error?cause.message:'生产运行状态更新失败。')}}
  ...
  return <PageContainer>
    <PageHeader title="AI 获客" description="统一使用搜索、地图、官网、行业名录和采购公告发现客户，并推进至客户与跟进闭环。" actions={<>
      ...
      <Button variant="primary" onClick={() => setDialog('task')}><Plus size={16} />创建获客</Button>
    </>} />
    ...
    {production&&production.state!=='not_configured'&&<Card size="small" title="生产运行控制" extra={<Space size={8}><Badge tone={production.state==='running'&&production.readyToSend?'green':production.readyToSend?'orange':'red'}>{production.state==='running'&&production.readyToSend?'自动运行中':production.state==='paused'?'已暂停':'等待就绪'}</Badge><Popconfirm title={production.state==='running'?'暂停全部自动触达？':'恢复全部自动触达？'} description={production.state==='running'?'待发送的自动消息会立即取消，已经发送的消息不会撤回。':'仅在发件、收件和安全阈值检查通过后恢复。'} okText={production.state==='running'?'暂停':'恢复'} cancelText="取消" onConfirm={()=>changeGlobalAutomation(production.state==='running'?'pause_all':'resume_all')}><Button size="sm" variant={production.state==='running'?'danger':'primary'} disabled={production.state!=='running'&&!production.readyToSend}>{production.state==='running'?'全部暂停':'恢复自动运行'}</Button></Popconfirm></Space>}>
      <Row gutter={[16,16]}>
    ...
    {currentPlan&&performance&&<Card size="small" title="自动化成效 · 近 30 天" extra={<Badge tone={performance.health==='healthy'?'green':'orange'}>{performance.health==='healthy'?'运行正常':'需要关注'}</Badge>}>
      <Row gutter={[16,16]}>
        <Col xs={12} md={6}><Statistic title="自动准入" value={performance.metrics.promoted} suffix={`/ ${performance.metrics.candidates} 家`}/><Typography.Text type="secondary">合格率 {performance.metrics.qualificationRate}%</Typography.Text></Col>
        <Col xs={12} md={6}><Statistic title="已发送首触达" value={performance.metrics.outreachSent} suffix="封"/><Typography.Text type="secondary">送达率 {performance.metrics.deliveryRate}%</Typography.Text></Col>
        <Col xs={12} md={6}><Statistic title="客户回复" value={performance.metrics.replies} suffix="条"/><Typography.Text type="secondary">回复率 {performance.metrics.replyRate}%</Typography.Text></Col>
        <Col xs={12} md={6}><Statistic title="自动形成商机" value={performance.metrics.deals} suffix="个"/><Typography.Text type="secondary">高意向 {performance.metrics.highIntent} 条</Typography.Text></Col>
      </Row>
      <div style={{marginTop:16}}><StatusNotice tone={performance.recommendations[0]?.level==='warning'?'warning':performance.recommendations[0]?.level==='success'?'success':'info'} title={performance.recommendations[0]?.title??'等待形成复盘样本'} description={performance.recommendations[0]?.description}/></div>
      {performance.sources[0]&&<Typography.Text type="secondary" style={{display:'block',marginTop:10}}>{performance.sources[0].promoted||performance.sources[0].replies?`当前贡献最高来源：${performance.sources[0].source} · ${performance.sources[0].promoted} 家自动准入 · ${performance.sources[0].replies} 条回复`:`当前候选主要来源：${performance.sources[0].source} · ${performance.sources[0].candidates} 家候选，尚未形成自动准入样本`}</Typography.Text>}
      <Flex gap={16} wrap align="center" style={{marginTop:10}}>
        <Space size={6} wrap>
          <Badge tone="blue">文案实验</Badge>
          <Typography.Text type="secondary">{performance.copyExperiment.status==='waiting'?'等待发送样本':performance.copyExperiment.status==='collecting'||!performance.copyExperiment.winner?'正在均衡分流':`正在向${performance.copyExperiment.variants.find(item=>item.variant===performance.copyExperiment.winner)?.label??'优胜版本'}倾斜`}</Typography.Text>
          {performance.copyExperiment.variants.map(item=><Typography.Text type="secondary" key={item.variant}>{item.label} {item.sent} 封 / 回复 {item.replyRate}%</Typography.Text>)}
        </Space>
        {performance.sources[0]&&<Space size={6} wrap><Badge tone={performance.sources[0].priority==='优先'?'green':performance.sources[0].priority==='探索'?'orange':'neutral'}>来源调优</Badge><Typography.Text type="secondary">{performance.sources[0].source} · {performance.sources[0].priority} · 质量分 {performance.sources[0].priorityScore}</Typography.Text></Space>}
        {learning&&<Space size={6} wrap>
          <Badge tone={learning.status==='active'?'green':learning.status==='learning'?'blue':'neutral'}>画像学习</Badge>
          <Typography.Text type="secondary">{learning.status==='active'
            ? `已学习 ${learning.labeledOutcomes} 条结果 · ${learning.positiveFeatures[0]?`正向 ${learning.positiveFeatures[0].label} +${learning.positiveFeatures[0].adjustment}`:'正向特征积累中'} · ${learning.riskFeatures[0]?`风险 ${learning.riskFeatures[0].label} ${learning.riskFeatures[0].adjustment}`:'风险特征积累中'} · 上限 ±${learning.maxScoreAdjustment}`
            : learning.message}</Typography.Text>
        </Space>}
        {activeLearningVersion&&<Typography.Text type="secondary">当前学习版本 v{activeLearningVersion.version}</Typography.Text>}
      </Flex>
    </Card>}
    ...
    <Panel>
        <TableToolbar filters={<>
            ...
            <CustomSelect ariaLabel="筛选候选范围" value={archiveView?'archived':'active'} onChange={value=>setArchiveView(value==='archived')} options={[{value:'active',label:'当前候选'},{value:'archived',label:'已归档'}]}/>
            ...
            <Button disabled={!archiveView&&candidateTaskId==='all'&&!query&&filter==='全部'&&source==='全部来源'&&sort==='匹配分最高'} onClick={()=>{setArchiveView(false);setCandidateTaskId('all');setQuery('');setFilter('全部');setSource('全部来源');setSort('匹配分最高')}}>清除筛选</Button>
          </>} selection={checkedCandidates.size>0?<SelectionBar count={checkedCandidates.size} unit="家候选" actions={<><Button disabled={archiveView} onClick={saveChecked}><Bookmark/>保存客户</Button><Button disabled={archiveView} onClick={()=>setBatchTaskOpen(true)}><CheckCircle2/>创建跟进</Button><Button onClick={()=>archiveCandidates([...checkedCandidates],!archiveView)}><Layers3/>{archiveView?'恢复所选':'归档所选'}</Button><Button aria-label="取消选择" title="取消选择" onClick={()=>setCheckedCandidates(new Set())}><X/></Button></>}/>:undefined}/>
    ...
    <CreateDialog open={dialog==='task'} title="创建获客计划" description="设置一次后，系统会按计划自动发现、去重并研究客户；每天打开系统即可查看新结果。" submitLabel="创建并运行" successMessage="获客计划已保存" onClose={()=>setDialog(null)} onSubmit={createRadarTask} initialValues={{icp:selectedMarketName,knowledge:`全部已启用知识（${enabledKnowledge} 条）`,strategy:'目标企业发现',dataSources:'搜索引擎,地图,行业目录,展会协会,采购公告',intentSignals:'采购公告,扩张建设,招聘变化,新闻融资',region:profileQuery.data?.regions||'全球',language:'自动识别',depth:'标准研究',limit:'100',schedule:'工作日自动',runTime:'08:00',automation:'自动研究'}} fields={[{name:'name',label:'计划名称',required:true,placeholder:'例如：欧洲卫生级泵阀经销商开发'},{name:'icp',label:'目标客户',type:'select',required:true,options:targetMarkets},{name:'region',label:'目标地区',required:true},{name:'limit',label:'每轮候选上限',type:'number',required:true},{name:'schedule',label:'运行周期',type:'select',required:true,options:['工作日自动','每天自动','每周一自动','仅运行一次']},{name:'runTime',label:'运行时间',required:true,description:'使用当前账户时区；服务重启或错过时间会自动补跑。'},{name:'automation',label:'自动化程度',type:'select',required:true,options:['自动研究','安全自动推进','全自动开发（安全发送）'],description:'全自动开发仅对高分、多证据、已验证且未退订的邮箱生成个性化首触达；工作时段错峰发送，未回复时最多跟进 2 次，客户一旦回复即停止，30 天内不会重复触达。'},{name:'strategy',label:'获客策略',type:'select',options:strategyOptions,advanced:true},{name:'dataSources',label:'使用的数据源',type:'multiselect',options:dataSourceOptions,placeholder:'留空时使用推荐数据源',advanced:true},{name:'intentSignals',label:'关注的意向信号',type:'multiselect',options:intentSignalOptions,advanced:true},{name:'knowledge',label:'参考的业务知识',type:'select',options:[`全部已启用知识（${enabledKnowledge} 条）`,'仅产品与客户判断规则','产品、案例与市场知识','不引用增长知识'],advanced:true},{name:'language',label:'研究语言',type:'select',options:['自动识别','中文','英语','德语'],advanced:true},{name:'depth',label:'研究深度',type:'select',options:['快速发现','标准研究','深度核验'],advanced:true},{name:'seedUrls',label:'补充公开网址',type:'textarea',placeholder:'每行一个官网、目录、展会或采购来源网址',advanced:true}]} />
    <CreateDialog open={dialog==='edit'} title="编辑获客计划" description="修改后从下一轮开始生效；正在运行的本轮不会被中断。" submitLabel="保存计划" successMessage="获客计划已更新" onClose={()=>setDialog(null)} onSubmit={updateCurrentPlan} initialValues={currentPlan?{name:currentPlan.name,region:currentPlan.targetRegion,limit:String(currentPlan.candidateLimit),schedule:currentPlan.scheduleType==='daily'?'每天自动':currentPlan.scheduleType==='weekly'?'每周一自动':currentPlan.scheduleType==='manual'?'仅运行一次':'工作日自动',runTime:currentPlan.runTimeLocal,automation:currentPlan.autoOutreachEnabled?'全自动开发（安全发送）':currentPlan.automationMode==='safe_autopilot'?'安全自动推进':'自动研究'}:{}} fields={[{name:'name',label:'计划名称',required:true},{name:'region',label:'目标地区',required:true},{name:'limit',label:'每轮候选上限',type:'number',required:true},{name:'schedule',label:'运行周期',type:'select',required:true,options:['工作日自动','每天自动','每周一自动','仅运行一次']},{name:'runTime',label:'运行时间',required:true},{name:'automation',label:'自动化程度',type:'select',required:true,options:['自动研究','安全自动推进','全自动开发（安全发送）']}]} />
    ...
    <DetailDrawer open={runHistoryOpen} width={720} title="运行记录" subtitle="查看客户发现进度、结果和执行日志" onClose={()=>setRunHistoryOpen(false)} footer={<>{activeTask&&['queued','running'].includes(activeTask.status)&&<Button onClick={()=>changeTaskStatus('pause')}>暂停运行</Button>}{activeTask?.status==='paused'&&<Button onClick={()=>changeTaskStatus('resume')}>继续运行</Button>}{activeTask?.status==='failed'&&<Button onClick={()=>changeTaskStatus('retry')}>重新运行</Button>}{activeTask&&['queued','running','paused','failed'].includes(activeTask.status)&&<Button variant="danger" onClick={()=>changeTaskStatus('cancel')}>取消运行</Button>}<Button onClick={()=>setRunHistoryOpen(false)}>关闭</Button><Button variant="primary" onClick={()=>{setRunHistoryOpen(false);setDialog('task')}}>{activeTask?'复制并新建':'创建获客'}</Button></>}>
      <Space orientation="vertical" size="middle" style={{width:'100%'}}>
        <DetailSection title="获客计划" subtitle="周期、健康状态和下一次自动运行" extra={currentPlan?<Badge tone={planTone(currentPlan.status)}>{planStatusLabels[currentPlan.status]}</Badge>:undefined}>
          {currentPlan?<><Descriptions bordered column={1} items={[{key:'schedule',label:'运行周期',children:`${scheduleLabels[currentPlan.scheduleType]} ${currentPlan.runTimeLocal}`},{key:'next',label:'下次运行',children:currentPlan.nextRunAt?new Date(currentPlan.nextRunAt).toLocaleString('zh-CN'):'按需运行'},{key:'runs',label:'累计运行',children:`${currentPlan.totalRuns} 轮`},{key:'automation',label:'自动化程度',children:currentPlan.autoOutreachEnabled?'高质量候选自动入库，并在安全门槛通过后执行有限触达序列；客户回复即停止':currentPlan.automationMode==='safe_autopilot'?'高质量候选自动入库，首次触达需人工批准':'自动研究，人工决定是否入库'},{key:'ai',label:'AI 门槛',children:currentPlan.requireAi?'AI 健康后自动运行':'允许基础研究'}]}/>{currentPlan.lastError&&<Alert type="warning" showIcon title={currentPlan.lastError} style={{marginTop:12}}/>}<Flex gap={8} wrap style={{marginTop:12}}><Button size="sm" onClick={()=>setDialog('edit')}>编辑计划</Button><Button size="sm" onClick={simulateCurrentPlan}>全流程演练</Button><Button size="sm" onClick={()=>changePlanStatus('run')}>立即运行</Button>{currentPlan.status==='paused'?<Button size="sm" onClick={()=>changePlanStatus('resume')}>恢复自动运行</Button>:<Button size="sm" onClick={()=>changePlanStatus('pause')}>暂停自动运行</Button>}</Flex></>:<EmptyState title="尚未建立持续获客计划" description="创建后系统会按周期自动运行。" icon={Clock3}/>}
        </DetailSection>
        ...
        {activeTask&&<>
          ...
          <DetailSection title="运行配置" subtitle="目标、地区与真实数据源">
            <Descriptions bordered column={{xs:1,sm:2}} items={[{key:'icp',label:'客户定位',children:activeTask.icp},{key:'region',label:'目标地区',children:activeTask.targetRegion},{key:'sources',label:'数据源',children:activeTask.dataSources.map(item=>dataSourceName[item]??item).join('、')},{key:'signals',label:'意向信号',children:activeTask.intentSignals.join('、')||'未指定'},{key:'depth',label:'研究深度',children:activeTask.depth},{key:'limit',label:'候选上限',children:`${activeTask.candidateLimit} 家`}]}/>
          </DetailSection>
