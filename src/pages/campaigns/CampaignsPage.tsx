  ...
  return (
    <div>
      ...
      <Panel>
        ...
        {campaignQuery.isLoading&&<div><RefreshCw className="is-spinning"/><strong>正在载入营销活动</strong><span>从当前工作区读取活动、执行节点与关联内容。</span></div>}
        {campaignQuery.isError&&<div><Target/><strong>营销活动载入失败</strong><span>{campaignQuery.error instanceof Error?campaignQuery.error.message:"请稍后重试。"}</span><Button onClick={()=>campaignQuery.refetch()}>重新加载</Button></div>}
        ...
        <DataTable
          ...
          rows={campaignPaging.pageItems.map(c=>{const meta=metaFor(c);const isPaused=c.status==="已暂停";return {key:c.id,className:checked.has(c.id)?"selected":"",cells:[
            <span><Checkbox aria-label={`选择 ${c.name}`} checked={checked.has(c.id)} onChange={e=>setChecked(current=>{const next=new Set(current);e.target.checked?next.add(c.id):next.delete(c.id);return next;})}/></span>,
            <Button onClick={()=>setSelected(c)}><i><Mail/></i><span><strong>{c.name}</strong><small>{c.market} · 受众 {meta.audience}</small></span></Button>,
            <div><Badge tone={c.status==="运行中"?"green":c.status==="草稿"?"orange":"neutral"}>{c.status}</Badge><small>{c.channel??meta.channel}</small></div>,
            <div><span><strong>{meta.progress}%</strong><small>执行进度</small></span><i><u style={{width:`${meta.progress}%`}}/></i></div>,
            <div><strong>{c.sent} 已触达</strong><small>{c.replies} 回复 · {meta.replyRate}</small></div>,
            <div><strong>{c.revenue}</strong><small>{c.opportunities} 个商机</small></div>,
            <div><strong>{meta.next}</strong><small>{c.status==="草稿"?"启动前需完成配置":"按当前活动节奏执行"}</small></div>,
            <div><Button aria-label={`查看 ${c.name}`} title="查看详情" onClick={()=>setSelected(c)}><ArrowRight/></Button><Button disabled={c.status==="草稿"} aria-label={isPaused?`恢复 ${c.name}`:`暂停 ${c.name}`} title={c.status==="草稿"?"草稿未启动":isPaused?"恢复活动":"暂停活动"} onClick={async()=>{const next=isPaused?"运行中":"已暂停";try{await campaignApi.update(c.id,{status:next});await queryClient.invalidateQueries({queryKey:["campaigns"]});showToast(`${c.name}${next==="已暂停"?"已暂停":"已恢复"}`)}catch(error){showToast(error instanceof Error?error.message:"活动状态更新失败")}}}>{isPaused?<Play/>:<Pause/>}</Button><Button aria-label={`更多操作：${c.name}`} title="更多操作" onClick={()=>{setSelected(c);setUtility("menu")}}><MoreHorizontal/></Button></div>,
          ]};})}
        />
        )}
        {visible.length > 0 && (
      ...
      <Modal open={dialog === "calendar"} title="活动排期" description="按时间查看当前工作区的活动执行节点。" onClose={()=>setDialog(null)} footer={<><Button onClick={()=>setDialog(null)}>关闭</Button><Button variant="primary" onClick={()=>setDialog("schedule-node")}><Plus/>添加节点</Button></>}>
        <div>{scheduleQuery.isLoading?<p>正在读取排期…</p>:scheduleQuery.data?.items.length?scheduleQuery.data.items.map(item=><article key={item.id}><i>{item.position}</i><span><strong>{item.name}</strong><small>{item.campaignName} · {item.scheduledAt?new Intl.DateTimeFormat("zh-CN",{dateStyle:"medium",timeStyle:"short"}).format(item.scheduledAt):"待安排"}</small></span><Badge tone={item.status==="completed"?"green":item.status==="cancelled"?"neutral":"blue"}>{item.status==="completed"?"已完成":item.status==="cancelled"?"已取消":"已排期"}</Badge></article>):<EmptyState title="暂无排期节点" icon={CalendarDays} />}</div>
      </Modal>
      <CreateDialog
      ...
      >
        <div>
          ...
          )) : <EmptyState title="暂无优化建议" icon={Sparkles} />}
        </div>
      </Modal>
      <Modal
      ...
      >
        <div>
          ...
          )) : <EmptyState title="暂无关联内容" icon={Mail} />}
        </div>
      </Modal>
      <Modal
        open={utility === "menu"}
