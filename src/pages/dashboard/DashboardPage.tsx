...
export function DashboardPage() {
  ...
  return <div>
    ...
    <section aria-label="今日经营摘要">
      ...
      <section aria-label="当前经营状态">
        {metrics.map(({label,value,detail,icon:Icon,change,progress})=><article key={label}><header><span>{label}</span><i><Icon size={17}/></i></header><div><strong>{value}</strong><b>{change}</b></div><footer><span>{detail}</span><i><u style={{width:`${progress}%`}}/></i></footer></article>)}
      </section>
    </section>

    <section aria-label="增长执行进度">
      ...
      <div>{workstreams.map(({label,detail,value,unit,progress,icon:Icon,path})=><Button key={label} onClick={()=>navigate(path)}><header><i><Icon/></i><span><strong>{label}</strong><small>{detail}</small></span><ArrowUpRight/></header><div><strong>{value}</strong><span>{unit}</span><b>{progress}%</b></div><footer><i><u style={{width:`${progress}%`}}/></i><span>本周完成度</span></footer></Button>)}</div>
    </section>

    <div>
      <Panel title="今日行动清单" subtitle="按紧迫度与收入影响排序，完成后自动更新经营状态" action={<Button size="sm" variant="ghost" onClick={()=>setDetails('tasks')}>全部事项<ArrowRight size={14}/></Button>}>
        <div>{taskItems.length ? taskItems.slice(0,4).map((t,index) => <article key={t[6]}><span>{String(index+1).padStart(2,'0')}</span><Button disabled={taskArchiveView} aria-label={`完成${t[1]}`} onClick={async()=>{try{await markComplete(t[6]);showToast(`${t[1]}已标记完成`)}catch(cause){showToast(cause instanceof Error?cause.message:'操作失败')}}}><CheckCircle2 size={18}/></Button><div><strong>{t[1]}</strong><p>{t[3]} · 下一步：{t[4]}</p></div><div><Badge tone={t[0] === '高' ? 'red' : 'orange'}>{t[0]}优先级</Badge><span>{t[2]}</span></div><div><small>预计影响</small><strong>{t[5]}</strong></div><Button aria-label={`更多操作：${t[1]}`} onClick={()=>{setActiveTask(t);setDetails('task')}}><MoreHorizontal size={18}/></Button></article>) : <EmptyState icon={ClipboardList} title={taskArchiveView?'暂无归档任务':'暂无待办事项'} />}</div>
      </Panel>

      <aside>
        <Panel title="经营提醒" subtitle="异常、机会和系统建议集中处理">
          <div>{attentionItems.map(({title,detail,count,tone,icon:Icon,path})=><Button key={title} onClick={()=>navigate(path)}><i><Icon/></i><span><strong>{title}</strong><small>{detail}</small></span><Badge tone={tone}>{count}</Badge><ArrowRight/></Button>)}</div>
          <div><i><Sparkles/></i><span><small>今日建议</small><strong>{focusCustomer?`优先推进 ${focusCompany}`:'先建立第一批客户'}</strong><p>{focusCustomer?`${focusAction}，并在完成后记录结果。`:'创建雷达任务并保存高匹配候选，形成可跟进客户。'}</p></span><Button onClick={()=>focusCustomer?setDetails('suggestion'):navigate('/radar')}>立即处理<ArrowRight/></Button></div>
        </Panel>
      </aside>
    </div>
