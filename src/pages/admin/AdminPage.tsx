...
export function AdminPage() {
  ...
  return <PageContainer>
    ...
    <Modal open={Boolean(selectedLog)} title="审计记录详情" description={selectedLog ? formatDate(selectedLog.createdAt, true) : ""} onClose={() => setSelectedLog(null)} footer={<Button onClick={() => setSelectedLog(null)}>关闭</Button>}>{selectedLog&&<Descriptions className="audit-detail-descriptions" bordered column={1} items={[["操作人", selectedLog.actor], ["操作", auditActionLabel(selectedLog.action)], ["对象", `${selectedLog.entityType} · ${selectedLog.entityId ?? "—"}`], ["类别", auditType(selectedLog.action)], ["结果", "成功"], ["IP 地址", selectedLog.ipAddress], ["记录编号", selectedLog.id]].map(item=>({key:item[0],label:item[0],children:item[1]}))}/>}</Modal>
  </PageContainer>;
}
