...
import { Avatar, Checkbox, Descriptions, Form, List, Space, Typography } from "antd";
import {
...
function Toolbar({ query, setQuery, filter, setFilter, options, sort, setSort, sortOptions, defaultSort, label, selectedCount, unit, onClear, actions, onRefresh, refreshing }: { query: string; setQuery: (value: string) => void; filter: string; setFilter: (value: string) => void; options: string[]; sort: string; setSort: (value: string) => void; sortOptions: string[]; defaultSort: string; label: string; selectedCount: number; unit: string; onClear: () => void; actions?: ReactNode; onRefresh: () => void; refreshing?: boolean }) {
  return <TableToolbar
    filters={<><SearchInput ariaLabel={`搜索${label}`} value={query} onChange={event => setQuery(event.target.value)} placeholder={`搜索${label}`}/><CustomSelect ariaLabel={`筛选${label}`} value={filter} onChange={setFilter} options={options}/><CustomSelect ariaLabel={`${label}排序`} value={sort} onChange={setSort} options={sortOptions}/><Button onClick={onRefresh} loading={refreshing}>{!refreshing&&<RefreshCw/>}刷新</Button><Button disabled={!query && filter === options[0] && sort === defaultSort} onClick={() => { setQuery(""); setFilter(options[0]); setSort(defaultSort); }}>清除筛选</Button></>}
    selection={selectedCount > 0 ? <SelectionBar summary={<Space><CheckCircle2/>已选择 {selectedCount} {unit}</Space>} actions={<>{actions}<Button aria-label="取消选择" title="取消选择" onClick={onClear}><X/></Button></>}/> : undefined}
  />;
}
...
function InvitationsSection({ items, loading, onRevoke }: { items: AdminInvitationApiRecord[]; loading: boolean; onRevoke: (id: string) => Promise<void> }) {
  ...
  return (
    <Panel title="待接受邀请" subtitle="邀请链接不会自动发送；复制链接并通过你的既有沟通渠道发送即可。">
      {loading ? <EmptyState title="正在读取邀请记录" icon={RefreshCw} spinning/> : pending.length ? <List dataSource={pending} renderItem={item=><List.Item actions={[<Button key="revoke" variant="danger" onClick={() => onRevoke(item.id)}>撤销</Button>]}><List.Item.Meta avatar={<UsersRound/>} title={item.displayName || item.email} description={`${item.email} · ${item.role === "admin" ? "管理员" : item.role === "viewer" ? "只读成员" : "成员"} · 截止 ${formatDate(item.expiresAt, true)}`}/></List.Item>}/> : <EmptyState title="暂无待接受邀请" description="生成邀请链接后会显示在这里。" icon={UsersRound}/>}
    </Panel>
  );
}
...
function ApprovalsSection({ items, loading, error, onRefresh, onReview }: { items: ApprovalApiRecord[]; loading: boolean; error: Error | null; onRefresh: () => void; onReview: (id: string, status: 'approved' | 'rejected') => Promise<void> }) {
  ...
  return <><Space><Button onClick={onRefresh}><RefreshCw size={14}/>刷新审批</Button></Space><DataTable columns={[{ key: 'request', title: '请求' }, { key: 'requester', title: '申请人' }, { key: 'status', title: '状态' }, { key: 'time', title: '提交时间' }, { key: 'actions', title: '操作' }]} rows={items.map(item => ({ key: item.id, cells: [<Space direction="vertical" size={0}><Typography.Text strong>{item.action}</Typography.Text><Typography.Text type="secondary">{item.entityType} · {item.entityId}</Typography.Text></Space>, <Typography.Text>{item.requester ?? item.requestedByUserId}</Typography.Text>, <Badge tone={item.status === 'pending' ? 'orange' : item.status === 'approved' ? 'green' : 'neutral'}>{item.status === 'pending' ? '待审批' : item.status === 'approved' ? '已通过' : item.status === 'rejected' ? '已驳回' : '已取消'}</Badge>, <Typography.Text>{formatDate(item.createdAt, true)}</Typography.Text>, item.status === 'pending' ? <Space><Button variant="primary" onClick={() => void onReview(item.id, 'approved')}>通过</Button><Button variant="danger" onClick={() => void onReview(item.id, 'rejected')}>驳回</Button></Space> : <Typography.Text>—</Typography.Text>] }))}/></>;
}
