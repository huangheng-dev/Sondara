...
function ApprovalsSection({ items, loading, error, onRefresh, onReview }: { items: ApprovalApiRecord[]; loading: boolean; error: Error | null; onRefresh: () => void; onReview: (id: string, status: 'approved' | 'rejected') => Promise<void> }) {
  if (loading) return <EmptyState className="list-empty-state" title="正在读取审批请求" icon={RefreshCw}/>;
  ...
  return <><div className="admin-toolbar customer-toolbar module-toolbar"><Button className="customer-refresh" onClick={onRefresh}><RefreshCw/>刷新审批</Button></div><DataTable className="customer-table customer-table-pro admin-customer-table" columns={[{ key: 'request', title: '请求' }, { key: 'requester', title: '申请人' }, { key: 'status', title: '状态' }, { key: 'time', title: '提交时间' }, { key: 'actions', title: '操作' }]} rows={items.map(item => ({ key: item.id, cells: [<div className="standard-cell-stack"><strong>{item.action}</strong><small>{item.entityType} · {item.entityId}</small></div>, <span>{item.requester ?? item.requestedByUserId}</span>, <Badge tone={item.status === 'pending' ? 'orange' : item.status === 'approved' ? 'green' : 'neutral'}>{item.status === 'pending' ? '待审批' : item.status === 'approved' ? '已通过' : item.status === 'rejected' ? '已驳回' : '已取消'}</Badge>, <span>{formatDate(item.createdAt, true)}</span>, item.status === 'pending' ? <span className="admin-inline-actions"><Button variant="primary" onClick={() => void onReview(item.id, 'approved')}>通过</Button><Button variant="danger" onClick={() => void onReview(item.id, 'rejected')}>驳回</Button></span> : <span>—</span>] }))}/></>;
}
