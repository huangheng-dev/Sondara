...
export function ContentPage() {
  ...
  return <div>
    ...
    <div>
      <section>
        <header><div><h2>选择创作方式</h2><p>选择信息来源和内容形式，再补充右侧生成设置</p></div></header>
        <div><section><div><span>生成手段</span><Badge tone="blue">{sourceMethods.length} 种</Badge></div><div>{sourceMethods.map(({label,icon:Icon})=><Button aria-pressed={sourceMethod===label} onClick={()=>{setSourceMethod(label);showToast(`已切换为${label}`)}} key={label}><Icon size={14}/><span>{label}</span></Button>)}</div></section><section><div><span>内容形式</span><Badge tone="blue">{activeTemplates.length} 种</Badge></div><CustomSelect ariaLabel="内容形式分类" value={creationGroup} onChange={setCreationGroup} options={creationGroups.map(group=>({value:group.label,label:group.label,icon:group.label==='客户触达'?<MessageSquareText/>:group.label==='专业内容'?<BookOpenText/>:group.label==='销售资料'?<ClipboardList/>:<Video/>}))}/><nav>{activeTemplates.map(({label,icon:Icon,desc})=><Button aria-pressed={template===label} onClick={()=>selectTemplate(label)} key={label}><i><Icon size={16}/></i><span><strong>{label}</strong><small>{desc}</small></span>{template===label&&<CheckCircle2 size={14}/>}</Button>)}</nav></section></div>
      </section>

      <div>
        <section><header><div><h2>编辑与发布</h2><p><span>{template}</span><small>{activeAssetId?'已载入服务端版本':'尚未保存'}</small></p></div><div><Button size="sm" onClick={()=>setPreview(value=>!value)}>{preview?'继续编辑':'预览'}</Button><Button size="sm" variant="primary" disabled={saving} onClick={saveDraft}>{saving?'正在保存…':'保存草稿'}</Button></div></header><Input aria-label="内容标题" value={title} onChange={e=>setTitle(e.target.value)}/><div><Button aria-label="加粗" onClick={()=>setEditor(value=>`**${value}**`)}><Bold size={15}/></Button><Button aria-label="插入链接" onClick={()=>{const url=window.prompt('请输入链接地址','https://');const text=window.prompt('请输入链接文本（可选）','')||url;if(url)setEditor(value=>`${value}\n\n[${text}](${url})`)}}><Link2 size={15}/></Button><span/><Button onClick={refineContent} disabled={generating}>{generating?<RefreshCw className="is-spinning" size={14}/>:<WandSparkles size={14}/>}AI 优化</Button><Button onClick={openLanguageCheck}><Languages size={14}/>语言检查</Button></div>{preview?<article><Badge tone="blue">预览</Badge><h2>{title}</h2>{editor.split('\n').map((line,index)=><p key={index}>{line||' '}</p>)}</article>:<Input.TextArea aria-label="内容编辑器" value={editor} onChange={e=>setEditor(e.target.value)}/>}<footer><span>{wordCount} 字 · {language} · 版本 {contentAssets.find(asset=>asset.id===activeAssetId)?.currentVersion??'未保存'}</span><div><Button onClick={async()=>{if(activeAssetId){const copy=await contentApi.duplicate(activeAssetId);loadAsset(copy);await queryClient.invalidateQueries({queryKey:['content-assets']});showToast('已创建并打开服务端副本')}else{setTitle(`${title}（副本）`);showToast('已创建未保存副本')}}}>创建副本</Button><Button size="sm" onClick={()=>{downloadText(`${title}.txt`,`${title}\n\n${editor}`);showToast('内容文档已下载')}}><Download size={14}/>导出</Button><Button size="sm" onClick={()=>{setCampaignAssetIds(null);setAssetDialog('campaign')}}><Send size={14}/>加入营销活动{linkedCampaigns>0?` · ${linkedCampaigns}`:''}</Button></div></footer></section>

        <aside><Panel title="生成设置" subtitle="按当前客户与业务上下文生成"><label>目标市场<CustomSelect ariaLabel="目标市场" value={market} onChange={setMarket} options={['德国食品设备','华东制药装备','北美阀门经销']}/></label><label>客户角色<CustomSelect ariaLabel="客户角色" value={role} onChange={setRole} options={['采购负责人','技术负责人','企业负责人']}/></label><label>购买阶段<CustomSelect ariaLabel="购买阶段" value={stage} onChange={setStage} options={['问题认知','方案比较','采购决策']}/></label><label>客户信号<CustomSelect ariaLabel="客户信号" value={signal} onChange={setSignal} options={['产品线扩张','新建产线','招投标活跃','访问产品页']}/></label><label>输出语言<CustomSelect ariaLabel="输出语言" value={language} onChange={setLanguage} options={['中文','English','Deutsch']}/></label><Button variant="primary" disabled={generating} onClick={generate}><WandSparkles size={15}/>{generating?'正在生成…':'根据设置重新生成'}</Button></Panel><div aria-label="内容质量详情"><header><div><h2>内容质量</h2><p>生成规则检查</p></div></header><section><b>{quality.overallScore}</b><span><strong>综合评分</strong><small>{quality.overallScore>=85?'良好 · 可进入编辑与审核':'建议优化后再进入审核'}</small></span></section><section>{[['客户相关性',quality.customerRelevance],['证据充分度',quality.evidenceScore],['行动清晰度',quality.actionClarity]].map(item=><div key={item[0]}><span>{item[0]}<b>{item[1]}</b></span><i><u style={{width:`${item[1]}%`}}/></i></div>)}</section><p><Target size={14}/>{quality.findings[0]??'内容结构完整，可进入审核。'}</p></div></aside>
      </div>
    </div>
    </Modal>
    ...
    <section>
      ...
      {contentQuery.isLoading?<div><RefreshCw className="is-spinning"/><strong>正在载入内容资产</strong><span>从当前工作区读取服务端内容与版本信息。</span></div>:contentQuery.isError?<div><CircleHelp/><strong>内容资产载入失败</strong><span>{contentQuery.error instanceof Error?contentQuery.error.message:'请稍后重试。'}</span><Button onClick={()=>contentQuery.refetch()}>重新加载</Button></div>:assetItems.length?<><DataTable
        ...
        rows={assetPaging.pageItems.map(item=>({key:item[5],className:selectedAssets.has(item[5])?'selected':'',cells:[
          <span><Checkbox aria-label={`选择 ${item[0]}`} checked={selectedAssets.has(item[5])} onChange={event=>setSelectedAssets(current=>{const next=new Set(current);event.target.checked?next.add(item[5]):next.delete(item[5]);return next})}/></span>,
          <Button onClick={()=>setSelectedAsset(item)}><i><FileText/></i><span><strong>{item[0]}</strong></span></Button>,
          <strong>{item[1]}</strong>,
          <span>{item[2]}</span>,
          <Badge tone={item[3]==='已发布'?'green':item[3]==='待审核'?'orange':'blue'}>{item[3]}</Badge>,
          <strong>{item[4]}</strong>,
          <div><Button aria-label={`打开${item[0]}`} title="查看内容" onClick={()=>setSelectedAsset(item)}><ChevronRight/></Button></div>,
        ]}))}
    ...
    <Modal open={assetDialog==='language'} title="语言与语气检查" description={`${language} · ${wordCount} 字`} onClose={()=>setAssetDialog(null)} footer={<><Button onClick={()=>setAssetDialog(null)}>关闭</Button><Button variant="primary" disabled={generating} onClick={async()=>{await refineContent();setAssetDialog(null)}}>应用优化</Button></>}><div>{checkingLanguage?<p>正在分析语言、语气和行动请求…</p>:languageTips.map(tip=><section key={tip.label}>{tip.tone==='good'?<CheckCircle2/>:<CircleHelp/>}<span><strong>{tip.label}</strong><small>{tip.detail}</small></span></section>)}</div></Modal>
    <Modal open={assetDialog==='versions'} title="历史版本" description="保留每次正文或标题变更，可追溯内容演进。" onClose={()=>{setAssetDialog(null);setVersionAssetId(null)}}><div>{versionsQuery.isLoading?<p>正在读取版本…</p>:versionsQuery.data?.items.length?versionsQuery.data.items.map(version=><article key={version.id}><FileText/><span><strong>版本 {version.versionNumber} · {version.changeNote}</strong><small>{new Intl.DateTimeFormat('zh-CN',{dateStyle:'medium',timeStyle:'short'}).format(version.createdAt)}</small></span><Badge tone="blue">{version.body.replace(/\s+/g,'').length} 字</Badge></article>):<EmptyState title="暂无历史版本" icon={FileText}/>}</div></Modal>
    <Modal open={Boolean(selectedAsset)} title={selectedAsset?.[0]??''} description={`${selectedAsset?.[1]??''} · ${selectedAsset?.[2]??''}`} onClose={()=>setSelectedAsset(null)} footer={<><Button onClick={()=>setSelectedAsset(null)}>关闭</Button><Button onClick={()=>{if(selectedAsset){setVersionAssetId(selectedAsset[5]);setSelectedAsset(null);setAssetDialog('versions')}}}>历史版本</Button><Button onClick={async()=>{if(!selectedAsset)return;await contentApi.update(selectedAsset[5],{status:'已归档'});setSelectedAsset(null);await queryClient.invalidateQueries({queryKey:['content-assets']});showToast('内容已归档')}}>归档</Button><Button variant="primary" onClick={()=>{if(selectedAsset)loadAsset(selectedAsset);setSelectedAsset(null);setCreatorOpen(true)}}>载入编辑器</Button></>}><div><Badge tone="blue">{selectedAsset?.[3]}</Badge><p>{selectedAsset?.[6]||'该内容可继续修改、复用或加入营销活动。'}</p><small>版本 {selectedAsset?.[7]} · 质量评分 {selectedAsset?.[8]} · 最近更新：{selectedAsset?.[4]}</small></div></Modal>
  </div>
}
