    ...
    <section className="content-assets content-assets-page standard-list-panel customer-workspace">
      <div className="customer-toolbar module-toolbar standard-list-toolbar content-asset-toolbar">
        <div className="customer-filter-controls content-asset-filter-controls">
          ...
          <Button className="customer-refresh" disabled={contentQuery.isFetching} onClick={async()=>{await contentQuery.refetch();showToast('内容资产已刷新')}}><RefreshCw className={contentQuery.isFetching?'is-spinning':undefined}/>刷新</Button>
      ...
      {contentQuery.isLoading?<div className="standard-list-state"><RefreshCw className="is-spinning"/><strong>正在载入内容资产</strong><span>从当前工作区读取服务端内容与版本信息。</span></div>:contentQuery.isError?<div className="standard-list-state"><CircleHelp/><strong>内容资产载入失败</strong><span>{contentQuery.error instanceof Error?contentQuery.error.message:'请稍后重试。'}</span><Button onClick={()=>contentQuery.refetch()}>重新加载</Button></div>:assetItems.length?<><DataTable
