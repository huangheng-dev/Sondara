...
import { campaignApi, collectAllPages, contentApi, icpApi, type ContentAssetApiRecord, type ContentAssetStatus, type ContentQualityResult } from '@/lib/api'
import { Card, Checkbox, Col, Flex, Form, Input, Progress, Row, Space, Typography } from 'antd'
...
export function ContentPage() {
  ...
  const selectedAssetRecords=contentAssets.filter(asset=>selectedAssets.has(asset.id))
  const selectedAssetsAreArchived=selectedAssetRecords.length>0&&selectedAssetRecords.every(asset=>asset.status==='已归档')
  ...
  const updateSelectedAssetStatus=async()=>{const nextStatus:ContentAssetStatus=selectedAssetsAreArchived?'草稿':'已归档';const targets=selectedAssetRecords.filter(asset=>asset.status!==nextStatus);if(!targets.length)return;try{await Promise.all(targets.map(asset=>contentApi.update(asset.id,{status:nextStatus})));setSelectedAssets(new Set());await queryClient.invalidateQueries({queryKey:['content-assets']});showToast(nextStatus==='已归档'?`已归档 ${targets.length} 个内容资产`:`已恢复 ${targets.length} 个内容资产为草稿`)}catch(error){showToast(error instanceof Error?error.message:'更新内容状态失败')}}
  ...
  return <PageContainer>
    ...
    <Modal className="content-creator-modal" open={creatorOpen} title="AI 创作" description="选择创作方式、生成内容并保存到内容资产。" width={1440} onClose={()=>{setCreatorOpen(false);setContextCustomer('')}}>
    ...
    <Panel>
      <TableToolbar filters={<>
        ...
        </>} selection={selectedAssets.size>0?<SelectionBar count={selectedAssets.size} unit="个内容资产" actions={<><Button disabled={selectedAssetsAreArchived} onClick={()=>{setCampaignAssetIds([...selectedAssets]);setAssetDialog('campaign')}}>加入活动</Button><Button onClick={()=>{const chosen=contentAssets.filter(asset=>selectedAssets.has(asset.id));downloadText('sondara-content-assets.txt',chosen.map(asset=>`${asset.title}\n\n${asset.body}`).join('\n\n---\n\n'));showToast(`已导出 ${chosen.length} 个内容资产`)}}>导出所选</Button><Button onClick={updateSelectedAssetStatus}>{selectedAssetsAreArchived?'恢复为草稿':'归档所选'}</Button><Button aria-label="取消选择" title="取消选择" onClick={()=>setSelectedAssets(new Set())}><X/></Button></>}/>:undefined}/>
      {contentQuery.isError?<PageState status="error" title="内容资产载入失败" description={contentQuery.error instanceof Error?contentQuery.error.message:'请稍后重试。'} onRetry={()=>contentQuery.refetch()}/>:<><DataTable
        ...
        columns={[
          ...
          {key:'title',title:<Button onClick={()=>setAssetSort(assetSort==='标题 A–Z'?'标题 Z–A':'标题 A–Z')}>内容标题{assetSortIcon(assetSort==='标题 A–Z'||assetSort==='标题 Z–A',assetSort==='标题 Z–A')}</Button>,width:260},
          {key:'market',title:<Button onClick={()=>setAssetSort('市场 A–Z')}>目标市场{assetSortIcon(assetSort==='市场 A–Z',false)}</Button>,width:165},
          {key:'role',title:'客户角色',width:145},
          {key:'status',title:'状态',width:105},
          {key:'updated',title:<Button onClick={()=>setAssetSort(assetSort==='最近更新'?'最早更新':'最近更新')}>更新时间{assetSortIcon(assetSort==='最近更新'||assetSort==='最早更新',assetSort==='最近更新')}</Button>,width:110},
          {key:'actions',title:'操作',width:63},
        ]}
