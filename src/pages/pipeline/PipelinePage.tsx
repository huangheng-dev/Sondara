...
import { Alert, Avatar, Card, Checkbox, Col, Descriptions, Flex, Progress, Row, Space, Statistic, Typography } from 'antd'
  ...
  return <PageContainer><PageHeader title="商机跟进" description="聚焦成交概率、风险和下一步动作，把商机从线索稳定推进到成交。" actions={<><Button onClick={()=>setArchiveView(value=>!value)}><Layers3 size={16}/>{archiveView?'返回进行中':'已归档'}</Button><Button onClick={()=>setForecastOpen(true)}><TrendingUp size={16}/>查看预测</Button><Button variant="primary" onClick={()=>setNewOpen(true)}><Plus size={16}/>新建商机</Button></>}/>
    <Panel>
      <TableToolbar filters={<>
          ...
          <Button loading={dealQuery.isFetching} onClick={async()=>{await dealQuery.refetch();showToast('商机列表已刷新')}}>{!dealQuery.isFetching&&<RefreshCw/>}刷新</Button>
          <Button disabled={!query&&owner==='全部负责人'&&stageFilter==='全部阶段'&&riskFilter==='全部风险'&&sort==='阶段概率最高'} onClick={clear}>清除筛选</Button>
        </>} selection={checked.size>0?<SelectionBar summary={<Space><CheckCircle2/>已选择 {checked.size} 个商机</Space>} actions={<>
      ...
      {dealQuery.isPending?<PageState status="loading" title="正在加载商机" description="正在读取当前工作空间的商机数据…"/>:dealQuery.isError?<PageState status="error" title="商机加载失败" description={dealQuery.error instanceof Error?dealQuery.error.message:'请确认 API 服务可用。'} onRetry={()=>dealQuery.refetch()}/>:items.length?<>
        <DataTable
          columns={[
            {key:'select',title:<Checkbox aria-label="选择本页全部商机" checked={dealPaging.pageItems.length>0&&dealPaging.pageItems.every(deal=>checked.has(deal.id))} onChange={event=>setChecked(current=>{const next=new Set(current);dealPaging.pageItems.forEach(deal=>event.target.checked?next.add(deal.id):next.delete(deal.id));return next})}/>,width:52},
          ...
          rows={dealPaging.pageItems.map(deal=>({key:deal.id,className:checked.has(deal.id)?'selected':'',cells:[
            <Checkbox aria-label={`选择 ${deal.company}`} checked={checked.has(deal.id)} onChange={event=>setChecked(current=>{const next=new Set(current);event.target.checked?next.add(deal.id):next.delete(deal.id);return next})}/>,
            <Button type="link" onClick={()=>setSelected(deal.index)}><Avatar>{deal.company.slice(0,1)}</Avatar><Space direction="vertical" size={0}><Typography.Text strong>{deal.company}</Typography.Text><Typography.Text type="secondary">{deal.source??'商机跟进'} · {deal.owner} 负责</Typography.Text><Typography.Text type="secondary"><BriefcaseBusiness size={13}/> 当前阶段：{deal.stage}</Typography.Text></Space></Button>,
            <Space direction="vertical" size={2}><Flex justify="space-between"><Badge tone={deal.probability>=80?'green':deal.probability>=60?'blue':'orange'}>{deal.stage}</Badge><Typography.Text strong>{deal.probability}%</Typography.Text></Flex><Progress aria-label={`${deal.company}成交概率`} percent={deal.probability} showInfo={false}/><Typography.Text type="secondary">当前阶段成交概率</Typography.Text></Space>,
            <Space direction="vertical" size={0}><Typography.Text strong>{deal.value}</Typography.Text><Typography.Text type="secondary">预计 {deal.close} 成交</Typography.Text></Space>,
            <Space direction="vertical" size={2}><Typography.Text strong><UserRound size={14}/> {deal.owner}</Typography.Text><Typography.Text type="secondary"><Clock3 size={13}/> 停留 {deal.age} 天</Typography.Text><Typography.Text type="danger"><AlertTriangle size={13}/> {deal.risk}</Typography.Text></Space>,
            <Button type="link" onClick={()=>setSelected(deal.index)}><CalendarDays/><Space direction="vertical" size={0}><Typography.Text strong>{deal.next}</Typography.Text><Typography.Text type="secondary">查看详情并记录结果</Typography.Text></Space></Button>,
            <Space><Button aria-label={`查看 ${deal.company}`} onClick={()=>setSelected(deal.index)}><Eye/></Button><Button aria-label={`推进 ${deal.company}`} disabled={archiveView||deal.stage==='赢单'} onClick={()=>advance(deal.company,deal.stage)}><ArrowRight/></Button><Button aria-label={`${archiveView?'恢复':'归档'} ${deal.company}`} onClick={()=>archiveDeals([deal.id],!archiveView)}><Layers3/></Button></Space>,
          ]}))}
        />
