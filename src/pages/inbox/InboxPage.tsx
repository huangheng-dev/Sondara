...
export function InboxPage() {
  ...
  const appliedSuggestionRef = useRef(new Set<string>())
  ...
  const replySuggestionQuery = useQuery({
    queryKey: ['inbox-reply-suggestion', activeThread?.id, activeThread?.lastInboundAt],
    queryFn: () => automationApi.replySuggestion(activeThread!.id),
    enabled: Boolean(activeThread?.id && activeThread.lastInboundAt),
    retry: false,
    staleTime: 10 * 60_000,
  })
  useEffect(() => {
  ...
  useEffect(() => {
    const suggestion = replySuggestionQuery.data
    if (!activeThread?.id || !suggestion?.draft || appliedSuggestionRef.current.has(activeThread.id)) return
    appliedSuggestionRef.current.add(activeThread.id)
    ...
    <Row gutter={[16, 16]}>
      ...
      <Col xs={24} xl={activeThread&&detailsOpen?11:17}>
        <Card className="inbox-conversation-card" title={activeThread ? `${activeThread.contact.name} · ${activeThread.contact.company}` : '对话'} extra={activeThread&&<Space><Badge tone={tone}>{activeThread.intent}</Badge><Button aria-label="查看客户详情" onClick={() => setDetailsOpen(true)}><UserRound /></Button></Space>}>
          {activeThread ? <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Card size="small" loading={replySuggestionQuery.isLoading} title={<Space><Sparkles/>AI 接管建议 · {activeThread.intent}</Space>} extra={replySuggestionQuery.data&&<Space size={6}><Badge tone={replySuggestionQuery.data.status==='ready'?'green':replySuggestionQuery.data.status==='fallback'?'orange':'neutral'}>{replySuggestionQuery.data.status==='ready'?'AI 已生成':replySuggestionQuery.data.status==='fallback'?'安全模板':'停止触达'}</Badge>{replySuggestionQuery.data.confidence>0&&<Typography.Text type="secondary">置信度 {replySuggestionQuery.data.confidence}%</Typography.Text>}</Space>}>
              {replySuggestionQuery.isError?<StatusNotice tone="warning" title="回复建议暂不可用" description="你仍可以人工编辑回复；发送前的确认和安全检查不会受到影响。"/>:replySuggestionQuery.data?.status==='blocked'?<StatusNotice tone="warning" title="当前不建议继续触达" description={replySuggestionQuery.data.nextAction}/>:<Space orientation="vertical" size={8} style={{width:'100%'}}>
