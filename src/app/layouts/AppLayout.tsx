...
import { AutoComplete, Avatar, Badge as AntBadge, Drawer, Dropdown, Empty, Flex, Grid, Input, Layout, Menu as AntMenu, Popover, Skeleton, Space, Typography, type InputRef, type MenuProps } from 'antd'
import { Bell, Check, ChevronRight, LogOut, Menu, PanelLeftClose, PanelLeftOpen, Search, Settings, ShieldCheck, UserRound } from 'lucide-react'
...
export function AppLayout() {
  ...
  const customerSearch = useQuery({
    queryKey: ['customers', authSession.data?.workspace.id, 'global-search', deferredQuery],
    queryFn: () => customerApi.list({ q: deferredQuery, pageSize: 10, sort: 'updated_desc' }),
    enabled: Boolean(authSession.data?.workspace.id && deferredQuery.length >= 2),
    retry: 1,
  })
  const inboxPreview = useQuery({ queryKey: ['inbox-threads', 'shell-preview'], queryFn: () => inboxApi.listThreads({ limit: 50 }), retry: 1 })
  const inboxSearch = useQuery({ queryKey: ['inbox-threads', 'global-search', deferredQuery], queryFn: () => inboxApi.listThreads({ q: deferredQuery, limit: 10 }), enabled: deferredQuery.length >= 2, retry: 1 })
  ...
  const notificationItems = notificationQuery.data?.items.slice(0, 8) ?? []
  ...
  const results = useMemo(() => {
    ...
    return [
      ...
      ...(inboxSearch.data?.items ?? inboxPreview.data?.items ?? []).filter(item => `${item.contact.name}${item.contact.company}${item.lastMessagePreview}${item.channel}`.toLowerCase().includes(keyword)).slice(0, 3).map(item => ({ title: item.contact.name, meta: `消息 · ${item.contact.company}`, path: '/inbox' })),
    ]
  }, [canManageWorkspace, customerSearch.data?.items, customers, deferredQuery, inboxPreview.data?.items, inboxSearch.data?.items])
  ...
  const notificationContent = <div className="notification-panel">
    <Flex className="notification-panel__header" align="center" justify="space-between" gap={16}>
      <Space orientation="vertical" size={1}>
        <Typography.Text className="notification-panel__title" strong>通知</Typography.Text>
        <Typography.Text className="notification-panel__status" type="secondary">
          {unreadCount ? `${unreadCount} 条未读` : notificationItems.length ? '消息均已读' : '暂无新通知'}
        </Typography.Text>
      </Space>
      {unreadCount > 0 && <Button className="notification-panel__mark-read" type="text" size="small" onClick={async() => { await automationApi.readAllNotifications(); await notificationQuery.refetch() }}>全部标为已读</Button>}
    </Flex>
    <div className="notification-panel__body">
      {notificationQuery.isLoading
        ? <Skeleton className="notification-panel__loading" active title={false} paragraph={{ rows: 3 }}/>
        : notificationItems.length
          ? notificationItems.map(item => <Button className={`notification-panel__item${item.readAt ? '' : ' is-unread'}`} type="text" block key={item.id} onClick={async() => { if(!item.readAt) await automationApi.readNotification(item.id); await notificationQuery.refetch(); navigate(item.actionPath || '/dashboard'); setNotificationsOpen(false) }}>
              <span className="notification-panel__dot" aria-hidden="true">{item.readAt ? <Check size={11}/> : null}</span>
              <span className="notification-panel__item-copy">
                <Typography.Text className="notification-panel__item-title" strong={!item.readAt} ellipsis>{item.title}</Typography.Text>
                <Typography.Text className="notification-panel__item-description" type="secondary">{item.description}</Typography.Text>
              </span>
              <ChevronRight className="notification-panel__item-arrow" size={15}/>
            </Button>)
          : <Empty className="notification-panel__empty" image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Space orientation="vertical" size={1}><Typography.Text type="secondary">暂无通知</Typography.Text><Typography.Text className="notification-panel__empty-hint" type="secondary">客户与任务提醒会显示在这里</Typography.Text></Space>}/>
      }
    </div>
    <div className="notification-panel__footer">
      <Button block type="text" onClick={() => { navigate('/dashboard'); setNotificationsOpen(false) }}>查看行动清单<ChevronRight size={14}/></Button>
    </div>
  </div>
    ...
    <Layout>
      <Header className="app-topbar" style={{ paddingInline: isDesktop ? 28 : 16 }}>
        ...
        <Flex className="app-topbar__end" align="center" gap={10}>
          <Popover classNames={{ root: 'notification-popover' }} content={notificationContent} trigger="click" placement="bottomRight" open={notificationsOpen} onOpenChange={setNotificationsOpen}><Button className="shell-action" aria-label={unreadCount ? `通知，${unreadCount} 条未读` : '通知'}><AntBadge count={unreadCount} size="small"><Bell size={19}/></AntBadge></Button></Popover>
