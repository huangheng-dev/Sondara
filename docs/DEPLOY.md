...
正式或个人部署不会预置登录账号。首次通过注册页创建的账户会成为工作区所有者，之后可在“账户设置 → 个人资料”修改邮箱和显示名称，在“账户设置 → 安全”修改密码。登录信息以当前 SQLite 数据库为准，不要把正式邮箱、密码或第三方密钥写入仓库和公共文档。

`npm run db:seed:dev` 只用于生成可公开演示的虚构账号和数据，不得用于正式数据库。切换数据库、批量清理或导入正式资料前，应先停止应用并独立备份当前 SQLite 文件，同时保存与该数据库配套的 `SONDARA_ENCRYPTION_KEY`。

仓库中的 `db:promote:formal`、`db:migrate:donjoy-company` 和 `flow:formal` 是历史业务迁移工具，包含特定公司和流程假设，不属于通用部署初始化步骤。只有在明确核对脚本目标、数据库路径和备份后才能使用；新部署应通过界面录入公司资料、配置 AI 模型与搜索服务，并按目标国家分别创建获客计划。
