...
const run = async () => {
  ...
  try {
    ...
    const whatsAppOptIn = await app.inject({ method: 'POST', url: `/api/customers/${customerId}/contacts/${contact.json().id}/whatsapp-opt-in`, headers: { cookie: ownerCookie }, payload: { optedIn: true, source: '书面同意' } })
    assert.equal(whatsAppOptIn.statusCode, 200, whatsAppOptIn.body)
    assert.ok(whatsAppOptIn.json().whatsappOptedInAt)
    ...
    const imported = await app.inject({ method: 'POST', url: '/api/customers/import', headers: { cookie: ownerCookie }, payload: { sourceName: '2026 汉诺威展商目录', sourceType: '展会名单', sourceUrl: 'https://example.com/exhibitors', rows: [
      { company: '展会客户 GmbH', region: '德国', industry: '工业设备', score: 82, confidence: 70, signal: '展会参展商', source: '忽略', size: '中型', stage: '待补全', contacts: 0, validContacts: 0, interaction: '忽略', nextAction: '验证联系人', estimatedValue: 0, contactName: 'Anna Schmidt', contactTitle: '采购总监', contactEmail: 'anna.schmidt@example.com' },
      { company: '展会客户 GmbH', region: '德国', industry: '工业设备', score: 82, confidence: 70, signal: '展会参展商', source: '忽略', size: '中型', stage: '待补全', contacts: 0, validContacts: 0, interaction: '忽略', nextAction: '验证联系人', estimatedValue: 0 },
    ] } })
    ...
    assert.equal(imported.json().contactsCreated, 1)
    const importedCustomer = (await app.inject({ method: 'GET', url: '/api/customers?q=展会客户', headers: { cookie: ownerCookie } })).json().items[0]
    assert.equal(importedCustomer.stage, '待验证')
    assert.match(importedCustomer.source, /2026 汉诺威展商目录/)
    const importHistory = await app.inject({ method: 'GET', url: '/api/customers/imports', headers: { cookie: ownerCookie } })
    assert.equal(importHistory.statusCode, 200, importHistory.body)
    assert.equal(importHistory.json().items[0].sourceName, '2026 汉诺威展商目录')

    const duplicateCustomer = await app.inject({ method: 'POST', url: '/api/customers', headers: { cookie: ownerCookie }, payload: { company: '闭环客户（重复）有限公司', region: '德国', industry: '工业设备', score: 81, signal: '重复导入' } })
    assert.equal(duplicateCustomer.statusCode, 201, duplicateCustomer.body)
    const duplicateContact = await app.inject({ method: 'POST', url: `/api/customers/${duplicateCustomer.json().id}/contacts`, headers: { cookie: ownerCookie }, payload: { name: 'Anna Meyer', jobTitle: '采购经理', phone: '+49 123 456', primaryChannel: 'WhatsApp' } })
    assert.equal(duplicateContact.statusCode, 201, duplicateContact.body)
    const mergePreview = await app.inject({ method: 'POST', url: '/api/customers/merge-preview', headers: { cookie: ownerCookie }, payload: { primaryCustomerId: customerId, duplicateCustomerId: duplicateCustomer.json().id } })
    assert.equal(mergePreview.statusCode, 200, mergePreview.body)
    assert.deepEqual(mergePreview.json().contacts.duplicateNames, ['Anna Meyer'])
    const merged = await app.inject({ method: 'POST', url: '/api/customers/merge', headers: { cookie: ownerCookie }, payload: { primaryCustomerId: customerId, duplicateCustomerId: duplicateCustomer.json().id } })
    assert.equal(merged.statusCode, 200, merged.body)
    const mergedContacts = await app.inject({ method: 'GET', url: `/api/customers/${customerId}/contacts`, headers: { cookie: ownerCookie } })
    assert.equal(mergedContacts.statusCode, 200, mergedContacts.body)
    assert.equal(mergedContacts.json().items.length, 1)
    assert.equal(mergedContacts.json().items[0].phone, '+49 555 0100')

    const leadConnection = await app.inject({ method: 'POST', url: '/api/lead-sources/connections', headers: { cookie: ownerCookie }, payload: { name: '闭环 Meta Lead Ads', provider: 'meta-lead-ads', accountRef: 'page-123', accessToken: 'meta-test-token' } })
    assert.equal(leadConnection.statusCode, 201, leadConnection.body)
    const webhookPath = new URL(leadConnection.json().webhookUrl).pathname + new URL(leadConnection.json().webhookUrl).search
    const webhook = await app.inject({ method: 'POST', url: webhookPath, payload: { leadgen_id: 'lead-123', entry: [] } })
    assert.equal(webhook.statusCode, 200, webhook.body)
    assert.equal(webhook.json().accepted, true)
    const duplicateWebhook = await app.inject({ method: 'POST', url: webhookPath, payload: { leadgen_id: 'lead-123', entry: [] } })
    assert.equal(duplicateWebhook.statusCode, 200, duplicateWebhook.body)
    assert.equal(duplicateWebhook.json().duplicate, true)
    const leadEvents = await app.inject({ method: 'GET', url: '/api/lead-sources/events', headers: { cookie: ownerCookie } })
    assert.equal(leadEvents.statusCode, 200, leadEvents.body)
    assert.equal(leadEvents.json().items[0].providerEventId, 'lead-123')
