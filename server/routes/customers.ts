...
const writeAudit = (workspaceId: string, actorUserId: string, action: string, entityId: string, metadata: unknown = {}) => {
  db.insert(auditLogs).values({ id: createId('aud'), workspaceId, actorUserId, action, entityType: 'customer', entityId, metadata: JSON.stringify(metadata), createdAt: Date.now() }).run()
}
...
export const customerRoutes: FastifyPluginAsync = async app => {
  ...
  app.get('/', async (request, reply) => {
    ...
    const total = db.select({ count: sql<number>`count(*)` }).from(customers).where(where).get()?.count ?? 0
    const names = new Map(db.select({ id: users.id, name: users.displayName }).from(users).all().map(item => [item.id, item.name]))
    const tags = db.select().from(customerTags).where(eq(customerTags.workspaceId, request.auth.workspaceId)).all()
    ...
    const items = db.select().from(customers).where(where).orderBy(orderBy).limit(query.pageSize).offset((query.page - 1) * query.pageSize).all().map(item => ({ ...item, ownerName: item.ownerUserId ? names.get(item.ownerUserId) ?? '未分配' : '未分配', tags: tagsByCustomer.get(item.id) ?? [] }))
  ...
  app.post('/', async (request, reply) => {
    ...
    if (requestedOwner && !db.select({ userId: workspaceMembers.userId }).from(workspaceMembers).where(and(eq(workspaceMembers.workspaceId, request.auth.workspaceId), eq(workspaceMembers.userId, requestedOwner))).get()) return reply.code(400).send({ error: 'INVALID_OWNER', message: '负责人不是当前工作区成员。' })
    ...
    try { db.insert(customers).values(record).run() } catch { return reply.code(409).send({ error: 'CUSTOMER_EXISTS', message: '该工作区已存在同名客户。' }) }
    writeAudit(request.auth.workspaceId, request.auth.userId, 'customer.created', record.id, { company: record.company })
  ...
  app.patch('/:id', async (request, reply) => {
    ...
    const existing = db.select().from(customers).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).get()
    ...
    if (changes.ownerUserId && !db.select({ userId: workspaceMembers.userId }).from(workspaceMembers).where(and(eq(workspaceMembers.workspaceId, request.auth.workspaceId), eq(workspaceMembers.userId, changes.ownerUserId))).get()) return reply.code(400).send({ error: 'INVALID_OWNER', message: '负责人不是当前工作区成员。' })
    db.update(customers).set({ ...changes, updatedAt: Date.now() }).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).run()
    writeAudit(request.auth.workspaceId, request.auth.userId, 'customer.updated', id, { fields: Object.keys(changes) })
    return db.select().from(customers).where(eq(customers.id, id)).get()
  })

  app.delete('/:id', async (request, reply) => {
    const id = (request.params as { id: string }).id
    const existing = db.select({ id: customers.id, company: customers.company }).from(customers).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).get()
    if (!existing) return reply.code(404).send({ error: 'NOT_FOUND', message: '客户不存在。' })
    db.delete(customers).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).run()
    writeAudit(request.auth.workspaceId, request.auth.userId, 'customer.deleted', id, { company: existing.company })
  ...
  app.post('/tags/bulk', async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'INVALID_INPUT', message: parsed.error.issues[0]?.message })
    const validCustomers = db.select({ id: customers.id }).from(customers).where(and(eq(customers.workspaceId, request.auth.workspaceId), inArray(customers.id, parsed.data.customerIds))).all()
    ...
    const now = Date.now()
    validCustomers.forEach(customer => db.insert(customerTags).values({ id: createId('ctg'), workspaceId: request.auth.workspaceId, customerId: customer.id, name: parsed.data.name, color: parsed.data.color, createdAt: now }).onConflictDoUpdate({ target: [customerTags.customerId, customerTags.name], set: { color: parsed.data.color } }).run())
    writeAudit(request.auth.workspaceId, request.auth.userId, 'customer.tags_added', validCustomers.map(item => item.id).join(','), { name: parsed.data.name, count: validCustomers.length })
    return reply.code(201).send({ updated: validCustomers.length })
  })

  app.get('/:id/contacts', async (request, reply) => {
    const id = (request.params as { id: string }).id
    const customer = db.select({ id: customers.id }).from(customers).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).get()
    if (!customer) return reply.code(404).send({ error: 'NOT_FOUND', message: '客户不存在。' })
    return { items: db.select().from(inboxContacts).where(and(eq(inboxContacts.workspaceId, request.auth.workspaceId), eq(inboxContacts.customerId, id))).all() }
  })

  app.post('/:id/contacts', async (request, reply) => {
    const id = (request.params as { id: string }).id
    const customer = db.select().from(customers).where(and(eq(customers.id, id), eq(customers.workspaceId, request.auth.workspaceId))).get()
    if (!customer) return reply.code(404).send({ error: 'NOT_FOUND', message: '客户不存在。' })
    ...
    try { db.insert(inboxContacts).values(record).run() } catch { return reply.code(409).send({ error: 'CONTACT_EXISTS', message: '该客户已存在同名联系人。' }) }
    const counts = db.select({ total: sql<number>`count(*)`, valid: sql<number>`sum(case when ${inboxContacts.email} is not null or ${inboxContacts.phone} is not null then 1 else 0 end)` }).from(inboxContacts).where(eq(inboxContacts.customerId, id)).get()
    db.update(customers).set({ contacts: counts?.total ?? 0, validContacts: counts?.valid ?? 0, updatedAt: now }).where(eq(customers.id, id)).run()
    writeAudit(request.auth.workspaceId, request.auth.userId, 'customer.contact_created', record.id, { customerId: id })
    return reply.code(201).send(record)
  })
}
