...
const writeAudit=async (ws:string,uid:string,action:string,et:string,eid:string,meta:Record<string,unknown>={})=>{
  await db.insert(auditLogs).values({id:createId("aud"),workspaceId:ws,actorUserId:uid,action,entityType:et,entityId:eid,metadata:JSON.stringify(meta),createdAt:Date.now()});
};
...
const aggregateChannels = async (ws: string, start: number, end: number, currency: string) => {
  const discoveredRows = (await db.select({ ch: radarCandidates.source, n: sql<number>`count(*)` })
  ...
  const qualifiedRows = (await db.select({ ch: customers.source, n: sql<number>`count(*)` })
      ...
      .groupBy(customers.source)) as { ch: string; n: number }[];
  const contactedRows = (await db.select({ ch: messageThreads.channel, n: sql<number>`count(distinct ${messageEntries.threadId})` })
      ...
      .groupBy(messageThreads.channel)) as { ch: string; n: number }[];
  const replyRows = (await db.select({ ch: messageThreads.channel, n: sql<number>`count(distinct ${messageEntries.threadId})` })
      .from(messageEntries).innerJoin(messageThreads, eq(messageEntries.threadId, messageThreads.id))
  ...
  const dealsRows = (await db.select({
      ...
      .groupBy(deals.source)) as { ch: string; total: number; won: number; revenue: number }[];
  const costRows = (await db.select({ ch: channelCosts.channel, cost: sql<number>`coalesce(sum(${channelCosts.costAmount}), 0)` })
      .from(channelCosts)
      ...
      .groupBy(channelCosts.channel)) as { ch: string; cost: number }[];
...
export const attributionRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get("/overview", async (request, reply) => {
    ...
    const channels = (await aggregateChannels(ws, start, end, currency));
  ...
  app.get("/costs", async (request, reply) => {
    ...
    const totalRow = (await db.$first(db.select({ n: sql<number>`count(*)` }).from(channelCosts).where(and(...filters))));
    const total = Number(totalRow?.n ?? 0);
    const items = (await db.select().from(channelCosts).where(and(...filters))
          .orderBy(desc(channelCosts.periodStart)).limit(pageSize).offset((page - 1) * pageSize));
  ...
  app.post("/costs", async (request, reply) => {
    ...
    await db.insert(channelCosts).values(record);
    await writeAudit(request.auth.workspaceId, request.auth.userId, "channel_cost.created", "channel_cost", record.id, { channel: record.channel, amount: record.costAmount });
  ...
  app.patch("/costs/:id", async (request, reply) => {
    ...
    await db.update(channelCosts).set({ ...updates, updatedAt: Date.now() }).where(eq(channelCosts.id, existing.id));
    const updated = (await db.$first(db.select().from(channelCosts).where(eq(channelCosts.id, existing.id))))!;
    await writeAudit(request.auth.workspaceId, request.auth.userId, "channel_cost.updated", "channel_cost", updated.id, { fields: Object.keys(updates) });
    return updated;
  });
  ...
  app.delete("/costs/:id", { preHandler: requirePermission('data.delete') }, async (request, reply) => {
    ...
    await writeAudit(request.auth.workspaceId, request.auth.userId, "channel_cost.deleted", "channel_cost", existing.id, { channel: existing.channel });
  ...
  app.post("/optimize-tasks", async (request, reply) => {
    ...
    const channels = (await aggregateChannels(ws, start, end, "CNY"));
    ...
    await writeAudit(ws, request.auth.userId, "attribution.optimize_tasks_created", "task_batch", created[0], { count: created.length, period, channels: targets.map(t => t.name) });
  ...
  app.get("/quality", async (request) => {
    const ws = request.auth.workspaceId;
    const customerTotal = Number((await db.$first(db.select({ n: sql<number>`count(*)` }).from(customers).where(eq(customers.workspaceId, ws))))?.n ?? 0);
    const customerWithDeal = Number((await db.$first(db.select({ n: sql<number>`count(distinct ${customers.id})` }).from(customers)
              .leftJoin(deals, eq(deals.customerId, customers.id))
              .where(and(eq(customers.workspaceId, ws), sql`${deals.id} is not null`))))?.n ?? 0);
    const threadTotal = Number((await db.$first(db.select({ n: sql<number>`count(*)` }).from(messageThreads).where(eq(messageThreads.workspaceId, ws))))?.n ?? 0);
    const threadWithChannel = Number((await db.$first(db.select({ n: sql<number>`count(*)` }).from(messageThreads)
              .where(and(eq(messageThreads.workspaceId, ws), sql`${messageThreads.channel} is not null`, sql`${messageThreads.channel} != ''`))))?.n ?? 0);
    const customerWithSource = Number((await db.$first(db.select({ n: sql<number>`count(*)` }).from(customers)
              .where(and(eq(customers.workspaceId, ws), sql`${customers.source} is not null`, sql`${customers.source} != ''`, sql`${customers.source} != '手动录入'`))))?.n ?? 0);
