...
const writeAudit = (workspaceId: string, actorUserId: string, action: string, entityId: string, metadata: unknown = {}) => {
  db.insert(auditLogs).values({ id: createId('aud'), workspaceId, actorUserId, action, entityType: 'deal', entityId, metadata: JSON.stringify(metadata), createdAt: Date.now() }).run()
}
...
export const dealRoutes: FastifyPluginAsync = async app => {
  ...
  app.get('/', async (request, reply) => {
    ...
    const total = db.select({ count: sql<number>`count(*)` }).from(deals).where(where).get()?.count ?? 0
    const items = db.select().from(deals).where(where).orderBy(orderBy).limit(query.pageSize).offset((query.page - 1) * query.pageSize).all()
  ...
  app.post('/', async (request, reply) => {
    ...
    const duplicate = db.select({ id: deals.id }).from(deals).where(and(eq(deals.workspaceId, request.auth.workspaceId), eq(deals.company, input.company))).get()
    if (duplicate) return reply.code(409).send({ error: 'DEAL_EXISTS', message: '该企业已经存在商机。' })
    const now = Date.now()
    let customer = input.customerId
      ? db.select().from(customers).where(and(eq(customers.id, input.customerId), eq(customers.workspaceId, request.auth.workspaceId))).get()
      : db.select().from(customers).where(and(eq(customers.workspaceId, request.auth.workspaceId), eq(customers.company, input.company))).get()
    ...
    db.transaction(tx => {
      if (!customer) {
        ...
        tx.update(customers).set({ stage: '有商机', estimatedValue: input.valueAmount, interaction: '刚刚 · 已创建商机', nextAction: input.nextAction, updatedAt: now }).where(eq(customers.id, customerId)).run()
      }
    ...
    writeAudit(request.auth.workspaceId, request.auth.userId, 'deal.created', dealId, { company: input.company, customerId })
    return reply.code(201).send(db.select().from(deals).where(eq(deals.id, dealId)).get())
  })
  ...
  app.patch('/:id', async (request, reply) => {
    ...
    const existing = db.select().from(deals).where(and(eq(deals.id, id), eq(deals.workspaceId, request.auth.workspaceId))).get()
    ...
    db.transaction(tx => {
      tx.update(deals).set({ ...changes, ...(probability !== undefined ? { probability } : {}), ...(stageChanged ? { stageEnteredAt: now } : {}), updatedAt: now }).where(and(eq(deals.id, id), eq(deals.workspaceId, request.auth.workspaceId))).run()
      if (existing.customerId) {
        ...
        }).where(and(eq(customers.id, existing.customerId), eq(customers.workspaceId, request.auth.workspaceId))).run()
      }
    })
    writeAudit(request.auth.workspaceId, request.auth.userId, 'deal.updated', id, { fields: Object.keys(changes) })
