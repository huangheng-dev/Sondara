...
const audit = (workspaceId: string, actorUserId: string, action: string, entityType: string, entityId: string, metadata: unknown = {}) => {
...
const serviceView = (workspaceId: string) => db.select({
...
}).from(aiServices).where(eq(aiServices.workspaceId, workspaceId)).orderBy(asc(aiServices.priority)).all()

export const aiServiceRoutes: FastifyPluginAsync = async app => {
  app.addHook('preHandler', requireAuth)

  app.get('/services', async request => ({ items: serviceView(request.auth.workspaceId) }))

  app.get('/policy', async request => {
    const policy = db.select().from(workspaceAiPolicies).where(eq(workspaceAiPolicies.workspaceId, request.auth.workspaceId)).get()
  ...
  app.patch('/policy', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    db.insert(workspaceAiPolicies).values(record).onConflictDoUpdate({
      target: workspaceAiPolicies.workspaceId,
      set: { ...parsed.data, updatedAt: record.updatedAt },
    }).run()
    audit(request.auth.workspaceId, request.auth.userId, 'ai.policy.updated', 'workspace_ai_policy', request.auth.workspaceId, parsed.data)
    return record
  })
  ...
  app.post('/services', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const now = Date.now()
    const nextPriority = parsed.data.priority ?? ((db.select({ max: sql<number>`coalesce(max(${aiServices.priority}), 0)` }).from(aiServices).where(eq(aiServices.workspaceId, request.auth.workspaceId)).get()?.max ?? 0) + 1)
    ...
    try { db.insert(aiServices).values(record).run() } catch { return reply.code(409).send({ error: 'SERVICE_EXISTS', message: '已存在同名 AI 服务。' }) }
    audit(request.auth.workspaceId, request.auth.userId, 'ai.service.created', 'ai_service', record.id, { provider: record.provider, model: record.model })
  ...
  app.patch('/services/:id', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const existing = db.select().from(aiServices).where(and(eq(aiServices.id, id), eq(aiServices.workspaceId, request.auth.workspaceId))).get()
    if (!existing) return reply.code(404).send({ error: 'NOT_FOUND', message: 'AI 服务不存在。' })
    db.update(aiServices).set({ ...parsed.data, updatedAt: Date.now() }).where(and(eq(aiServices.id, id), eq(aiServices.workspaceId, request.auth.workspaceId))).run()
    audit(request.auth.workspaceId, request.auth.userId, 'ai.service.updated', 'ai_service', id, { fields: Object.keys(parsed.data) })
    return serviceView(request.auth.workspaceId).find(item => item.id === id)
  })

  app.get('/services/:id/keys', async (request, reply) => {
    const id = (request.params as { id: string }).id
    const service = db.select({ id: aiServices.id }).from(aiServices).where(and(eq(aiServices.id, id), eq(aiServices.workspaceId, request.auth.workspaceId))).get()
    if (!service) return reply.code(404).send({ error: 'NOT_FOUND', message: 'AI 服务不存在。' })
    const items = db.select({ id: aiServiceKeys.id, serviceId: aiServiceKeys.serviceId, name: aiServiceKeys.name, ending: aiServiceKeys.ending, enabled: aiServiceKeys.enabled, failureCount: aiServiceKeys.failureCount, cooldownUntil: aiServiceKeys.cooldownUntil, lastUsedAt: aiServiceKeys.lastUsedAt, createdAt: aiServiceKeys.createdAt, updatedAt: aiServiceKeys.updatedAt }).from(aiServiceKeys).where(and(eq(aiServiceKeys.serviceId, id), eq(aiServiceKeys.workspaceId, request.auth.workspaceId))).orderBy(asc(aiServiceKeys.createdAt)).all()
    return { items }
  })
  ...
  app.post('/services/:id/keys', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'INVALID_INPUT', message: parsed.error.issues[0]?.message })
    const service = db.select({ id: aiServices.id }).from(aiServices).where(and(eq(aiServices.id, serviceId), eq(aiServices.workspaceId, request.auth.workspaceId))).get()
    if (!service) return reply.code(404).send({ error: 'NOT_FOUND', message: 'AI 服务不存在。' })
    ...
    db.insert(aiServiceKeys).values(record).run()
    audit(request.auth.workspaceId, request.auth.userId, 'ai.key.created', 'ai_service_key', record.id, { serviceId, ending: record.ending })
  ...
  app.patch('/keys/:id', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: 'INVALID_INPUT', message: parsed.error.issues[0]?.message })
    const key = db.select().from(aiServiceKeys).where(and(eq(aiServiceKeys.id, id), eq(aiServiceKeys.workspaceId, request.auth.workspaceId))).get()
    if (!key) return reply.code(404).send({ error: 'NOT_FOUND', message: '密钥不存在。' })
    db.update(aiServiceKeys).set({ enabled: parsed.data.enabled, updatedAt: Date.now() }).where(eq(aiServiceKeys.id, id)).run()
    audit(request.auth.workspaceId, request.auth.userId, 'ai.key.updated', 'ai_service_key', id, { enabled: parsed.data.enabled })
    return { id, enabled: parsed.data.enabled }
  })

  app.delete('/keys/:id', { preHandler: requireAdmin }, async (request, reply) => {
    const id = (request.params as { id: string }).id
    const key = db.select({ id: aiServiceKeys.id, serviceId: aiServiceKeys.serviceId, ending: aiServiceKeys.ending }).from(aiServiceKeys).where(and(eq(aiServiceKeys.id, id), eq(aiServiceKeys.workspaceId, request.auth.workspaceId))).get()
    if (!key) return reply.code(404).send({ error: 'NOT_FOUND', message: '密钥不存在。' })
    db.delete(aiServiceKeys).where(and(eq(aiServiceKeys.id, id), eq(aiServiceKeys.workspaceId, request.auth.workspaceId))).run()
    audit(request.auth.workspaceId, request.auth.userId, 'ai.key.deleted', 'ai_service_key', id, { serviceId: key.serviceId, ending: key.ending })
    return reply.code(204).send()
  })

  app.post('/services/:id/test', { preHandler: requireAdmin }, async (request, reply) => {
    const id = (request.params as { id: string }).id
    const service = db.select().from(aiServices).where(and(eq(aiServices.id, id), eq(aiServices.workspaceId, request.auth.workspaceId))).get()
    if (!service) return reply.code(404).send({ error: 'NOT_FOUND', message: 'AI 服务不存在。' })
    try {
      ...
      audit(request.auth.workspaceId, request.auth.userId, 'ai.service.tested', 'ai_service', id, { success: true, latency: result.latencyMs })
      return { ok: true, latencyMs: result.latencyMs }
    } catch (cause) {
      const message = cause instanceof Error ? cause.message : '连接失败'
      audit(request.auth.workspaceId, request.auth.userId, 'ai.service.tested', 'ai_service', id, { success: false })
