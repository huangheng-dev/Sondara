...
export const config = {
  ...
  externalConnectorWorkerEnabled: booleanFromEnv(process.env.SONDARA_EXTERNAL_CONNECTOR_WORKER_ENABLED, true),
  externalConnectorWorkerIntervalMs: numberFromEnv(process.env.SONDARA_EXTERNAL_CONNECTOR_WORKER_INTERVAL_MS, 30_000),
  salesGuardianEnabled: booleanFromEnv(process.env.SONDARA_SALES_GUARDIAN_ENABLED, true),
  salesGuardianIntervalMs: numberFromEnv(process.env.SONDARA_SALES_GUARDIAN_INTERVAL_MS, 60_000),
  ...
  linkedinApiVersion: process.env.SONDARA_LINKEDIN_API_VERSION?.trim() ?? "202606",
  metaGraphApiVersion: process.env.SONDARA_META_GRAPH_API_VERSION?.trim() ?? "v23.0",
  logLevel,
  isProduction,
